...
//Our aviator default ssl version should be tls 1 if server does not support tls 1 i.e if server only support upto ssl 3 version then aviator will show error massage rather than page. but in  reality it will .01% champion utpal  
const uint16 kDefaultSSLVersionMin =SSL_PROTOCOL_VERSION_SSL3; //champion  /*SSL_PROTOCOL_VERSION_TLS1; //block this for discarding ssl version 3 for aviator charmpion (utpal)*/
