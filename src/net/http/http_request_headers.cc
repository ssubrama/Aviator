...
#include "chrome/browser/championconfig/addreferrer/referrer_header_handler.h" // champion
#include "url/gurl.h"
...
/* champion - Add Referrer */
void HttpRequestHeaders::Insert(int allow, std::string pattern) {
  if(allow == 1)
    net::HttpUtil::AllowVector->push_back(pattern);
  else
    net::HttpUtil::BlockVector->push_back(pattern);	
}

void HttpRequestHeaders::RemoveException (int allow, std::string pattern) {
  std::vector<std::string> ::iterator myiterator;
  std::vector<std::string> ::iterator myiterator1;

  if(allow == 2) {		
    for(myiterator=net::HttpUtil::BlockVector->begin();myiterator!=net::HttpUtil::BlockVector->end();++myiterator) {
      std::string temp(*myiterator);
      if(temp == pattern ) {
        myiterator1=myiterator;
        net::HttpUtil::BlockVector->erase(myiterator1);
        break;
      }
    }
  } else {
    for(myiterator=net::HttpUtil::AllowVector->begin();myiterator!=net::HttpUtil::AllowVector->end();++myiterator) {
      std::string temp(*myiterator);
      if(temp == pattern ) {
        myiterator1=myiterator;
        net::HttpUtil::AllowVector->erase(myiterator1);
        break;
      }
    }
  }
