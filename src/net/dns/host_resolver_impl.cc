...
// Attempts to connect a UDP socket to |dest|:53. // champion Blocked for error: unused function 'IsGloballyReachable' [-Werror,-Wunused-function] (Balaji)
...
HostResolverImpl::Key HostResolverImpl::GetEffectiveKeyForRequest(
  ...
  if (info.address_family() == ADDRESS_FAMILY_UNSPECIFIED) {
    if (probe_ipv6_support_ && !use_local_ipv6_) {
      ...
      //This if added by utpal for strip outbound request to google public dns 
      //utpalendu for blocking google public dns server we need to block here -champion
#if 0
      base::TimeTicks start_time = base::TimeTicks::Now();
      // Google DNS address.
...
#endif
      //End of utpalendu and below line added for make it default address family no problem champion
      effective_address_family = default_address_family_;
    }
  }
