...
SInt32 nRes = 0;
 ...
 const void* vals[] = {
  CFSTR("NTLM Authentication Failed"),
  CFSTR("Due to a severe privacy leak in NTLM Aviator has blocked \nyour browser from connecting to this site for your safety") };
#endif  // defined(OS_MACOSX)
namespace net {
...
void URLRequest::NotifyAuthRequiredComplete(
  ...
  switch (result) {
    ...
    case NetworkDelegate::AUTH_REQUIRED_RESPONSE_CANCEL_NTML_ENABLE: {
      CancelAuth();
      #if defined(OS_WIN)
      HWND handleToAviator = GetActiveWindow();
      ::MessageBox(handleToAviator, L"Due to a severe privacy leak in NTLM, Aviator has blocked \nyour browser from connecting to this site for your safety.", L"NTLM Authentication Failed", MB_ICONERROR | MB_TOPMOST | MB_SETFOREGROUND | MB_APPLMODAL);
	  #endif
#if defined(OS_MACOSX)
