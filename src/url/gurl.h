...
class URL_EXPORT GURL {
 public:
  ...
  // Added for champion
  bool DomainCompare(const char* lower_ascii_domain, int domain_len) const;
 
 
  // If function DomainIs only has parameter lower_ascii_domain, which means
  // domain string should be terminate with NULL character.
  bool DomainCompare(const char* lower_ascii_domain) const {
    return DomainCompare(lower_ascii_domain,
                    static_cast<int>(strlen(lower_ascii_domain)));
  }
  // champion : end
  ...
  void setSpec(std::string spec){ // champion - navneet 
    spec_ = spec;
  } 
