...
GURL::GURL(const std::string& url_string) {
  // champion : open newtab page to homepage (navneet)
  std::string homePage = HOMEPAGE_URL;
  if (url_string == "aviator://newtab" || url_string == "aviator://newtab/")
    InitCanonical(homePage, true);
  else
    // champion : open newtab page to homepage (navneet)
...
GURL::GURL(const base::string16& url_string) {
  // champion : open newtab page to homepage (navneet)
 // base::string16 homepage = TEXT(HOMEPAGE_URL);
 base::string16 homepage = base::ASCIIToUTF16(HOMEPAGE_URL);
  if (url_string.compare(base::ASCIIToUTF16("aviator://newtab"))== 0  || url_string.compare(base::ASCIIToUTF16("aviator://newtab/")) == 0)
    InitCanonical(homepage, true);
  else
    InitCanonical(url_string, true);
}
...
// champion
bool GURL::DomainCompare(const char* lower_ascii_domain,
                    int domain_len) const {
  // Return false if this URL is not valid or domain is empty.
