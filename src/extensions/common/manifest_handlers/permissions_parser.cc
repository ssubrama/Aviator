...
bool CanSpecifyHostPermission(const Extension* extension,
  ...
  if (!pattern.match_all_urls() &&
    ...
    // Component extensions can have access to all of aviator://*.
    ...
    // extensions which request aviator:// urls
    return false;
  }
  ...
  for (std::vector<std::string>::const_iterator iter = host_data.begin();
    ...
    if (parse_result == URLPattern::PARSE_SUCCESS) {
      ...
      if (pattern.scheme() != content::kChromeUIScheme &&
          !PermissionsData::CanExecuteScriptEverywhere(extension)) {
        // Keep aviator:// in allowed schemes only if it's explicitly requested
      ...
      // We need to make sure all_urls matches aviator://favicon and (maybe)
      // aviator://thumbnail, so add them back in to host_permissions separately.
      if (pattern.match_all_urls()) {
