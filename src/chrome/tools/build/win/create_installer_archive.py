...
CHROME_DIR = "Aviator-bin" # champion : branding (amresh)
...
def GetPrevVersion(build_dir, temp_dir, last_chrome_installer, output_name):
  ...
  cmd = [lzma_exec,
         ...
         '-o"%s"' % temp_dir,
         prev_archive_file,
         'Aviator-bin/*/Aviator.dll',] # champion : branding (amresh)
  RunSystemCommand(cmd)
  dll_path = glob.glob(os.path.join(temp_dir, 'Aviator-bin', '*', 'Aviator.dll')) # champion : branding (amresh)
...
def _ParseOptions():
  ...
  parser.add_option('-s', '--skip_rebuild_archive',
      default="False", help='Skip re-building Aviator.7z archive if it exists.')
  ...
  parser.add_option('-a', '--diff_algorithm', default='BSDIFF',
      help='Diff algorithm to use when generating differential patches '
           '{BSDIFF|COURGETTE}.')
  parser.add_option('-n', '--output_name', default='Aviator',
  ...
  parser.add_option('--component_build', default='0',
      help='Whether this archive is packaging a component build. This will '
           'also turn off compression of Aviator.7z into Aviator.packed.7z and '
           'helpfully delete any old Aviator.packed.7z in |output_dir|.')
