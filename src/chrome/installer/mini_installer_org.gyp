{
  ...
  'conditions': [
    ['OS=="win"', {
      'targets': [
        {
          'target_name': 'AviatorSetup',
          'type': 'executable',

          'dependencies': [
            '../chrome.gyp:Aviator', # champion : branding (amresh)
          ...
          'msvs_settings': {
            ...
            'VCManifestTool': {
              'AdditionalManifestFiles': [
                '$(ProjectDir)\\mini_installer\\AviatorSetup.exe.manifest',
              ],
            },
          },
          'configurations': {
          ...
          'rules': [
            ...
            },
            {
              ...
              'inputs': [
                '<(create_installer_archive_py_path)',
                '<(PRODUCT_DIR)/Aviator.exe', # champion : branding (amresh)
                '<(PRODUCT_DIR)/Aviator.dll', # champion : branding (amresh)
                '<(PRODUCT_DIR)/nacl64.exe',
                '<(PRODUCT_DIR)/ppGoogleNaClPluginChrome.dll',
                '<(PRODUCT_DIR)/nacl_irt_x86_32.nexe',
                '<(PRODUCT_DIR)/nacl_irt_x86_64.nexe',
