...

{
  'targets': [
          ...
          'actions': [
            {
              ...
              'inputs': [
                '<(reorder_py_path)',
                '$(OutDir)\\initialexe\\Aviator.exe',
              ],
              'outputs': [
                '<(PRODUCT_DIR)\\Aviator.exe',
                '<(PRODUCT_DIR)\\aviator.exe.pdb',
              ],
              'action': [
                'python',
                '<(reorder_py_path)',
    ...
    },
    {
      ...
      'type': 'executable',
      # Name the exe Aviator.exe, not chrome_initial.exe.
      ...
      'conditions': [
        ...
        ['OS=="win"', {
          'dependencies': [
            ...
            '../chrome_elf/chrome_elf.gyp:Aviator_elf',
          ...
          'msvs_settings': {
            'VCLinkerTool': {
              ...
              'OutputFile': '$(OutDir)\\initialexe\\Aviator.exe',
              'DelayLoadDLLs': [
                'dbghelp.dll',
                'dwmapi.dll',
                'uxtheme.dll',
            ...
            'VCManifestTool': {
              'AdditionalManifestFiles': [
                '$(ProjectDir)\\app\\Aviator.exe.manifest',
