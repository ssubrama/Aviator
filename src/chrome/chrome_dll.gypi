      ...
      'targets': [
              ...
              'actions': [
                {
                  ...
                  'inputs': [
                    '$(OutDir)\\initial\\Aviator.dll', # champion : branding (amresh)
                  ],
                  'outputs': [
                    '$(OutDir)\\Aviator.dll', # champion : branding (amresh)
                  ],
                  'action': ['tools\\build\\win\\hardlink_failsafe.bat',
                             '$(OutDir)\\initial\\Aviator.dll',
                             '$(OutDir)\\Aviator.dll'],
                },
              ],
              'conditions': [
                  ...
                  'actions': [
                    {
                      ...
                      'outputs': [
                        '$(OutDir)\\aviator.dll.pdb',
                      ],
                      'action': ['tools\\build\\win\\hardlink_failsafe.bat',
                                 '$(OutDir)\\initial\\aviator.dll.pdb',
                                 '$(OutDir)\\aviator.dll.pdb'],
                    }
                  ]
                }]
        ...
        },
        {
          ...
          'conditions': [
            ['OS=="win"', {
              'dependencies': [
                '<(DEPTH)/chrome_elf/chrome_elf.gyp:Aviator_elf',
              ],
            }],
            ...
            ['OS=="win"', {
              'product_name': 'Aviator', # champion : branding (amresh)
              'dependencies': [
              ...
              'msvs_settings': {
                'VCLinkerTool': {
                  ...
                  'conditions': [
                    ['incremental_chrome_dll==1', {
                      'OutputFile': '$(OutDir)\\initial\\Aviator.dll', # champion : branding (amresh)
                ...
                'VCManifestTool': {
                  'AdditionalManifestFiles': [
                    '$(ProjectDir)\\app\\Aviator.dll.manifest',
                  ],
                },
              },
              'conditions': [
      ...
      'targets': [
        {
          ...
          'type': 'shared_library',
          'product_name': 'Aviator_child', # champion : branding (amresh)
          'variables': {
            'enable_wexit_time_destructors': 1,
          },
          'dependencies': [
