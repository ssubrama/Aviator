...
extern const char kAviatorRestarted[]; // champion
#if defined(OS_WIN)
...
extern const char kWarnBeforeQuitEnabled[]; // champion warn_before_quit_enabled
extern const char kRestartManual[];////champion Dinesh for Manual Restart
extern const char kNotificationKeyChainAccess[]; //champion  : keychain notification (navneet)
extern const char kDefaultSearchPopUp[]; // champion Disconnect default search popup (navneet)
/* Referrer preference Variables  - champion */
extern const char kManagedDefaultReferrerSetting[];  /* Champion - Added for manage exception of referrer header feature */ 
extern const char kManagedReferrerAllowedForUrls[]; /* Champion - Added for manage exception of referrer header feature */ 
extern const char kManagedReferrerBlockedForUrls[];
extern const char kAllwaysOrNeverReferrersHeader[]; //Added for champion purge_referrer

/*  NPCC preference variables - champion */
extern const char kConnectionControlRules[]; //Champion Added for Network port connection control  
extern const char kLoadInDefaultBrowser[]; // Load in default browser - Champion
extern const char kDisplayAlternateBrowser[]; // Display Alternate Browser of blocked urls NPCC feature - Champion 
extern const char kNtlmControlRules[];		//This is added for ntlm champion:( utpal )
/* champion preference variables ends here */

extern const char kRulesMovedToPref[]; // This is to move the pre defined rules from Varsconfig to preference - champion
extern const char kNPCCRulesUpdated[];
}  // namespace prefs
