...
// even though aviator://thumb (and everything that's not aviator://favicon with
// a aviator:// scheme) is not a valid permission.
...
TEST(PermissionsTest, ChromeURLs) {
  ...
  allowed_hosts.AddPattern(
      URLPattern(URLPattern::SCHEME_ALL, "aviator://favicon/"));
  allowed_hosts.AddPattern(
      URLPattern(URLPattern::SCHEME_ALL, "aviator://thumb/"));
  scoped_refptr<PermissionSet> permissions(
