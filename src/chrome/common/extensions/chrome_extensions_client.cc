...
void ChromeExtensionsClient::FilterHostPermissions(
  ...
  for (URLPatternSet::const_iterator i = hosts.begin();
       i != hosts.end(); ++i) {
    // Filters out every URL pattern that matches aviator:// scheme.
    if (i->scheme() == content::kChromeUIScheme) {
      // aviator://favicon is the only URL for aviator:// scheme that we
...
URLPatternSet ChromeExtensionsClient::GetPermittedChromeSchemeHosts(
  ...
  URLPatternSet hosts;
  // Regular extensions are only allowed access to aviator://favicon.
  hosts.AddPattern(URLPattern(URLPattern::SCHEME_CHROMEUI,
                              chrome::kChromeUIFaviconURL));

  // Experimental extensions are also allowed aviator://thumb.
  //
  ...
  // make aviator://thumbs available to NTP Russia extension as
  // non-experimental.
