...
#include "chrome/browser/championconfig/varsconfig.h" /* Champion : Added for library options */
namespace prefs {
...
#if defined(OS_MACOSX)
// Set before restarting Aviator for update mechanism champion
const char kAviatorRestarted[] = "aviator.restart";
...
// remote debugging at aviator://inspect.
...
/* ------------------------- champion preference variables starts here --------------------------------------------*/

// Boolean that is true for Before quit the browser ask for warning else not asking warning quit
//without ask confirmation
const char kWarnBeforeQuitEnabled[] = "profile.warn_before_quit_enabled"; // // champion added for warn_before_quit
const char kRestartManual[] = "profile.restart_manual";//champion Dinesh for Manual Restart
const char kNotificationKeyChainAccess[] ="profile.notification_keychain_enabled"; // champion : keychain notification (navneet) 
const char kDefaultSearchPopUp[] = "profile.default_search_popup"; // champion Disconnect default search popup (navneet) 
//Added for purge referrer managed exception champion
const char kAllwaysOrNeverReferrersHeader[] ="AllwaysSend_NeverSend";

/* Champion - Added for manage exception of referrer header feature */ 

const char kManagedReferrerAllowedForUrls[] =
    "profile.managed_referrer_allowed_for_urls";
const char kManagedReferrerBlockedForUrls[] =
    "profile.managed_referrer_blocked_for_urls";
// Set to true if the user created a login item so we should not modify it when
// uninstalling background apps.

const char kManagedDefaultReferrerSetting[] =            /* Champion - Added for manage exception of referrer header feature */ 
    "profile.managed_default_content_settings.referrer";
    
/* Load in default browser feature - Champion */
const char kLoadInDefaultBrowser[] = 
	"profile.LoadInDefaultBrowser";

/* Display Alternate Browser of blocked urls NPCC feature - Champion */
const char kDisplayAlternateBrowser[] = 
	"profile.display_alternate_browser";
const char kConnectionControlRules[]="profile.ConnectionControlRules"; //Champion Added for Network port connection control 

//This below pref variable is added for ntlm utpal (champion)
//A boolean pref set default false for subpress NTLM for http
const char kNtlmControlRules[] = "profile.kNtlmControlRules"; //This is added for ntlm champion:( utpal )
const char kNPCCRulesUpdated[] = "profile.NPCCRulesUpdated"; 
/* ------------------------- champion preference variables ends here --------------------------------------------*/

const char kRulesMovedToPref[] = "profile.RulesMovedToPref"; // This is to confirm whether the predefined rules are moved to preference
