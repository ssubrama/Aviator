...
void NetErrorHelperCore::OnFinishLoad(FrameType frame_type) {
  ...
  }else if (auto_reload_enabled_ && // champion - Added condition to disable auto reload of blocked URLs by NPCC - Siva

             IsReloadableError(*committed_error_page_info_) && committed_error_page_info_->error.reason != net::ERR_BLOCKED_BY_ADMINISTRATOR) {
...
bool NetErrorHelperCore::MaybeStartAutoReloadTimer() {
  if (!committed_error_page_info_ ||
   ...
   if (committed_error_page_info_ && committed_error_page_info_->error.reason == net::ERR_BLOCKED_BY_ADMINISTRATOR) /* champion - Disabling Reload for URLs blocked by NPCC */
