...
void RenderViewContextMenu::AppendDeveloperItems() {
  ...
  // Champion artf414382 Build library on Mac 	  
  // Champion  artf360043 :	Remove Inspect Element menu items 
  //if((!INSPECT_ELEMENT)||SINGLE_SITE)
  if(INSPECT_ELEMENT) {	
    menu_model_.AddSeparator(ui::NORMAL_SEPARATOR);
...
void RenderViewContextMenu::AppendLinkItems() {
  if (!params_.link_url.is_empty()) {
    ...
    /* Champion - Sinlge site option through library */
...
void RenderViewContextMenu::AppendAudioItems() {
  ...
  /* Champion - Single site through library */
  if (!SINGLE_SITE || WANT_WRENCH_MENU ) {                                
...
void RenderViewContextMenu::AppendPageItems() {
  ...
  if (TranslateService::IsTranslatableURL(params_.page_url)) {
    ...
    /* Champion - Sinlge site through library */													   
    if (!SINGLE_SITE) {                                                       
      menu_model_.AddItem(
  ...
  /* Champion - Added compile time option to remove the developer tools in context menu */
  if (!DEVELOPER_TOOL_FROM_RIGHT) { //||!SINGLE_SITE)
...
void RenderViewContextMenu::AppendFrameItems() {
  ...
  /* Champion - Disabled developer tools from right menu by default */
  if (!DEVELOPER_TOOL_FROM_RIGHT) {
...
bool RenderViewContextMenu::IsCommandIdEnabled(int id) const {
  ...
  switch (id) {
    ...
    case IDC_CONTENT_CONTEXT_TRANSLATE: {
      ...
      // language.
      /* Champion artf414382 Build library on Mac  */
      /* Champion artf379494 : To Disable Translate into English from main menu */
      if (DISABLE_TRANSLATE_TO_ENGLISH) {
