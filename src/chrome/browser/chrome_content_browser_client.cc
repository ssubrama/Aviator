...
#include "chrome/browser/championconfig/varsconfig.h" //Added for Champion
...
bool HandleWebUI(GURL* url, content::BrowserContext* browser_context) {
  ...
  if (!url->host().empty()) {
    ...
    // Handle valid "aviator://aviator/foo" URLs so the reverse handler will
    // be called.
...
// Reverse URL handler for Web UI. Maps "aviator://aviator/foo/" to
// "aviator://foo/".
...
void ChromeContentBrowserClient::RegisterProfilePrefs(
  ...
  /* - WebGL is disabled by default in Champion browser */
  if(DISABLE_WEBGL) {
    ...
    registry->RegisterBooleanPref(
        prefs::kDisable3DAPIs,
        false,
        user_prefs::PrefRegistrySyncable::UNSYNCABLE_PREF); // chrome default
  }   // End Champion
  registry->RegisterBooleanPref(
...
bool ChromeContentBrowserClient::ShouldAllowOpenURL(
  ...
  // Do not allow aviator://aviator-signin navigate to other aviator:// URLs, since
...
#if defined(OS_ANDROID)
  // Handler to rewrite aviator://newtab on Android.
  handler->AddHandlerPair(&chrome::android::HandleAndroidNewTabURL,
                          BrowserURLHandler::null_handler());
#else
  // Handler to rewrite aviator://newtab for InstantExtended.
...
void ChromeContentBrowserClient::GetAdditionalMappedFilesForChildProcess(
  ...
  base::FilePath chrome_resources_pak =
      data_path.AppendASCII("Aviator_100_percent.pak"); // champion : app name changes (amresh)
...
void ChromeContentBrowserClient::PreSpawnRenderer(
  ...
  sandbox::ResultCode result = policy->AddRule(
      ...
      L"\\\\.\\pipe\\Aviator.nacl.*"); // champion : sandboxing for XP (amresh)
