...
#include "chrome/browser/ui/browser.h" // Champion Added for WBQ Start (Balaji)
...
- (void)terminate:(id)sender {
  ...
  if(!brestartAviatorToTakeUpdate)
  	[appController tryToTerminateApplication:self];
  else
    [appController tryToTerminateApplicationAndRestart:self]; // Champion RestartAviatorDialog restart browser when update is available (Balaji)
...
- (BOOL)sendAction:(SEL)anAction to:(id)aTarget from:(id)sender {
  ...
  if (enableNSExceptions)
   ...
   bCloseSingleTabAndWindow=false; //  Champion WBQ  (Balaji) 
