...
bool WillHandleBrowserAboutURL(GURL* url,
  ...
  *url = url_fixer::FixupURL(url->possibly_invalid_spec(), std::string());
  ...
  // Only handle aviator://foo/, url_fixer::FixupURL translates about:foo.
...
#endif
  // Redirect aviator://settings
  } else if (host == chrome::kChromeUISettingsHost) {
    host = chrome::kChromeUIUberHost;
    path = chrome::kChromeUISettingsHost + url->path();
  // Redirect aviator://help
...
bool HandleNonNavigationAboutURL(const GURL& url) {
  ...
  // aviator://ipc/ is currently buggy, so we disable it for official builds.
