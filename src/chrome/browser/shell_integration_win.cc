...
#include "base/files/file_path.h" // champion balaji
#include "chrome/common/importer/firefox_importer_utils.h" // balaji
...
bool DirExistsIE(const std::string& dirName_in) {
...
std::string ShellIntegration::GetDefaultBrowserForAviator() {
  if( base::win::GetVersion() >= base::win::VERSION_VISTA) {
    ...
    int pos=strProogID_IE.find("IE.HTTP");
    ...
    if(pos!=std::string::npos) { //default browser IE
      ...
      if (DirExistsIE (strCheckFolderFor64bitWin))
...
bool ShellIntegration::IsSafariDefaultBrowser() {
  bool ff_default = false;
  if (base::win::GetVersion() >= base::win::VERSION_VISTA) {
    base::string16 app_cmd;
    base::win::RegKey key(HKEY_CURRENT_USER,
    ...
    base::string16 app_cmd;
    if (key.Valid() && (key.ReadValue(L"", &app_cmd) == ERROR_SUCCESS) &&
        base::string16::npos != StringToLowerASCII(app_cmd).find(L"safari"))
      ff_default = true;
  }
  return ff_default;
}

bool ShellIntegration::IsGoogleChromeDefaultBrowser() {
  bool ff_default = false;
  if (base::win::GetVersion() >= base::win::VERSION_VISTA) {
    base::string16 app_cmd;
    base::win::RegKey key(HKEY_CURRENT_USER,
...
bool ShellIntegration::IsIEDefaultBrowser() {
  bool ff_default = false;
  if (base::win::GetVersion() >= base::win::VERSION_VISTA) {
    base::string16 app_cmd;
    base::win::RegKey key(HKEY_CURRENT_USER,
