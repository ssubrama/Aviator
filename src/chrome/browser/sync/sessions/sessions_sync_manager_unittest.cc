...
// Test that we exclude tabs with only aviator:// and file:// schemed navigations
// from ShouldSyncTab(..).
TEST_F(SessionsSyncManagerTest, ValidTabs) {
  ...
  // A aviator:// entry isn't valid.
  content::NavigationEntry* entry(content::NavigationEntry::Create());
  entry->SetVirtualURL(GURL("aviator://preferences/"));
...
TEST_F(SessionsSyncManagerTest, AssociateWindowsDontReloadTabs) {
  ...
  AddTab(browser(), GURL("aviator://preferences/"));
  InitWithSyncDataTakeOutput(syncer::SyncDataList(), &out);
...
TEST_F(SessionsSyncManagerTest, NotifiedOfRefresh) {
  ...
  NavigateAndCommitActiveTab(GURL("aviator://newtab/#open_tabs"));
