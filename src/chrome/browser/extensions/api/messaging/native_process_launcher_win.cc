...
const wchar_t kNativeMessagingRegistryKey[] =
    L"SOFTWARE\\WhiteHat\\Aviator\\NativeMessagingHosts"; // champion : sandboxing for XP (amresh)
...
bool NativeProcessLauncher::LaunchNativeProcess(
  ...
  base::string16 out_pipe_name = base::StringPrintf(
      L"\\\\.\\pipe\\Aviator.nativeMessaging.out.%llx", pipe_name_token); // champion : sandboxing for XP (amresh)
  base::string16 in_pipe_name = base::StringPrintf(
      L"\\\\.\\pipe\\Aviator.nativeMessaging.in.%llx", pipe_name_token); // champion : sandboxing for XP (amresh)
