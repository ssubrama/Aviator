...
void ExtensionService::DisableUserExtensions(
  ...
  // champion : handling through our code (navneet)
  ...
  // champion navneet : To stop User Agent Switcher to be disabled when clicking on reset button start
  const ExtensionSet& championDisabled_set = registry_->disabled_extensions();
  for (ExtensionSet::const_iterator extension = championDisabled_set.begin();
	  extension != championDisabled_set.end(); ++extension) {
	  if ((*extension)->name() == "User-Agent Switcher for Chrome"){
  ...
  // champion navneet : To stop User Agent Switcher to be disabled when clicking on reset button end
  for (extensions::ExtensionList::const_iterator extension = to_disable.begin();
      extension != to_disable.end(); ++extension) {
    if ((*extension)->was_installed_by_default() &&
        extension_urls::IsWebstoreUpdateUrl(
...
void ExtensionService::NotifyExtensionLoaded(const Extension* extension) {
  ...
  // If the extension has permission to load aviator://favicon/ resources we need
  ...
  // Same for aviator://theme/ resources.
  ...
  // Same for aviator://thumb/ resources.
...
void ExtensionService::AddExtension(const Extension* extension) {
  ...
  // champion : set extension state enabled for PDF Viewer & Disconnect (balaji)
  if(/*extension->name() == "PDF Viewer" || */
     extension->name() == "Disconnect" || 
     extension->name() == "User-Agent Switcher for Chrome")
    extension_prefs_->SetExtensionState(extension->id(), Extension::ENABLED);
