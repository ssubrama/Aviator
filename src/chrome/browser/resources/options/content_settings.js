...
cr.define('options', function() {
  ...
  ContentSettings.setContentFilterSettingsValue = function(dict) {
    for (var group in dict) {
      ...
      var controlledBy = managedBy == 'policy' || managedBy == 'extension' ?
          managedBy : null;
      // champion : we have disabled some of controls and hence this fix (amresh)
      if (document.querySelector('input[type=radio][name=' + group + '][value=' +
                             dict[group].value + ']') != null)
  ...
  ContentSettings.setOTRExceptions = function(type, otrExceptions) {
    ...
    // champion : we have disabled some of controls and hence this fix (amresh)
    if (exceptionsList.parentNode != null)
