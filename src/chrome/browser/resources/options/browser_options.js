...
<if expr="is_macosx">
      //notification key chain champion
      $('notification-enabled').onclick = function(event) {
         var state = $('notification-enabled').checked;
         chrome.send('notificationKeyChainEnabled',[Boolean(state)]);
      }
      BrowserOptions.notificationKeyChainEnabledJS = function(enable) {
      $('notification-enabled').checked = enable;
      }
</if>
	...
	/*  OptionsPage.navigateToPage('alternateBrowser');  */ // champion
      ...
      //champion
      $('privacyAddReferrerButton').onclick = function (event) {        
        OptionsPage.navigateToPage('referrer');        
        chrome.send('coreOptionsUserMetricsAction',
            ['Options_ReferrerHeader']);
      };
      
      $('connectionControlButton').onclick = function (event) {        
        OptionsPage.navigateToPage('NetworkPortConnection');        
	chrome.send('coreOptionsUserMetricsAction',
	    ['Options_connectioncontrol']);
      };
	    
      $('set-as-default-browser').onload = function (event) {        	       
      };
      
      /*$('set-as-default-browser').onclick = function (event) {        
        OptionsPage.navigateToPage('alternateBrowser');        
	chrome.send('coreOptionsUserMetricsAction',
	    ['Options_AlternateBrowser']);
      };*/
      ...
      // champion navneet test -- not required to hide as we want to show this option even in private mode
    ...
    */
    ...
    onHomePageChanged_: function(event) {
      if (!event.value.uncommitted)
        ...
        /* champion - added to set search engine from home page URL */
      var engineSelect = $('default-search-engine');
      var HomeURL = 'whitehatsec.com/securebrowser';
      //var HomeURL = 'whitehat.photoninfotech.com/whitehat/securebrowser.html';
      var settingsHomeURL = $('home-page-url').textContent;
      
      if (settingsHomeURL.search(HomeURL)) {
        var found = settingsHomeURL.search('#searchEngine=');
        var URLSearchengine = settingsHomeURL.substr(found+14);
        if (URLSearchengine == 'Yahoo' || URLSearchengine == 'yahoo')
          URLSearchengine = URLSearchengine + '! (Disconnect)';
        else
          URLSearchengine = URLSearchengine + ' (Disconnect)';
      
        for (var i = 0; i < engineSelect.length; i++) {
          if (URLSearchengine.toLowerCase() == engineSelect.options[i].text.toLowerCase())
            chrome.send('setDefaultSearchEngine', [String(engineSelect.options[i].value)]);
          }      
        }   /* champion - End */
    },
    ...
    */
    ...
    updateSearchEngines_: function(engines, defaultValue, defaultManaged) {
      ...
      var defaultIndex = -1;
      /* champion - sorting the default search engine */
      var sortedEngines = [], firstEngine;
      for(var i=0; i < engineCount; i++) {
        var engine = engines[i];	
        if(engine.name.toLowerCase().indexOf('google') >=0 && engine.name.toLowerCase().indexOf('(disconnect)') >= 0){
          firstEngine = engine;
        } else if(engine.name.toLowerCase().indexOf('(disconnect)') >= 0){
          sortedEngines.unshift(engine);
        } else {
      ...
      if(firstEngine) {
        sortedEngines.unshift(firstEngine);
      }

      for (var i = 0; i < engineCount; i++) {
        var engine = sortedEngines[i];
	/* champion - sorting the default search engine end */
   ...
   /**
     ...
     *       profileInfo = {
     *         name: "Profile Name",
     *         iconURL: "aviator://path/to/icon/image",
    ...
    */
    ...
    updateAccountPicture_: function() {
      var picture = $('account-picture');
      if (picture) {
        picture.src = 'aviator://userimage/' + this.username_ + '?id=' +
            Date.now();
      }
    },
    ...
    toggleExtensionControlledBox_: function(
      ...
      div.style.backgroundImage =
          'url(aviator://extension-icon/' + extensionId + '/24/1)';
