  ...
  ExtensionsList.prototype = {
    ...
    createNode_: function(extension) {
      ...
      if (!incognito.disabled) {
        // champion Allow protected always true for Disconnect (Balaji)
        if(extension.name =="Disconnect") {
          incognito.addEventListener('change', function(e) {
	    // champion Allow protected always true (Balaji)
            var checked = true; // e.target.checked;
	    incognito.checked=checked;
	...
	// champion Rest of the user installed extensions will behave user inputs(Allow protected) (Balaji)
      	else {  
	  incognito.addEventListener('change', function(e) {
