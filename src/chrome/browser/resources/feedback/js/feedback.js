...
/**
 * Opens a new window with aviator://slow_trace, downloading performance data.
 */
function openSlowTraceWindow() {
  chrome.app.window.create(
      'aviator://slow_trace/tracing.zip#' + feedbackInfo.traceId);
}
      ...
      chrome.feedbackPrivate.getStrings(function(strings) {
        ...
        if ($('sys-info-url')) {
          ...
          $('sys-info-url').onclick =
              windowOpener(SYSINFO_WINDOW_ID, 'aviator://system');
        }
        if ($('histograms-url')) {
          // Opens a new window showing the histogram metrics.
          $('histograms-url').onclick =
              windowOpener(STATS_WINDOW_ID, 'aviator://histograms');
        }
      });
    }
  });
