...
#include "chrome/browser/championconfig/connectioncontrol/control_ntml_http_auth.h" // champion : ntlm (utpal)
...
PrefService* g_pref_service_for_ntlm = NULL; // champion : ntlm (utpal)
...
void ChromeNetworkDelegate::InitializePrefsOnUIThread(
  ...
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  g_pref_service_for_ntlm = pref_service; // champion : ntlm (utpal)
...
ChromeNetworkDelegate::OnAuthRequired(
  ...
  //get schme name 
  std::string authMode = auth_info.scheme;
  if (authMode.compare("ntlm") || authMode.compare("NTLM"))   {
    //Get the remote ip addresss and port for connection
    std::string url = request->url().spec();
    net::HostPortPair ipAddressPort = request->GetSocketAddress();
    uint16 port = request->GetSocketAddress().port();
    std::string host = request->GetSocketAddress().host();
    std::string domain_name = auth_info.challenger.host();
    //get remote ip address from hostport
    std::string remoteAddress = ipAddressPort.HostForURL();
    control_ntml_http_auth_.reset(new NTLMControl::ControlNtlmHttpAuth());
    control_ntml_http_auth_->setIpAddress(const_cast<std::string &>(remoteAddress));
    control_ntml_http_auth_->setPortNumber(port);
    control_ntml_http_auth_->setHost(host);
    control_ntml_http_auth_->setRequestUrl(url);
    control_ntml_http_auth_->setDomainName(domain_name);
   
    //need to check the url status i.e need to block or allow 
    if (!control_ntml_http_auth_->IsUrlAllowed()) {
      return net::NetworkDelegate::AUTH_REQUIRED_RESPONSE_CANCEL_NTML_ENABLE;
    }
  }
  //end of champion
