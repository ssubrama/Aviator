...
std::string ProcessMemoryInformation::GetRendererTypeNameInEnglish(
  ...
  std::string strBrowserName(BROWSER_NAME);	// champion
  switch (type) {
    case RENDERER_NORMAL:
      return "Tab";
    case RENDERER_CHROME:
      //return "Tab (Aviator)"; // champion modified to change aviator://memory Tab (Chrome) to Tab (Aviator)
      return "Tab ("+strBrowserName+")";
...
void MemoryDetails::CollectChildInfoOnUIThread() {
  ...
  for (size_t index = 0; index < chrome_browser->processes.size();
    ...
    while (content::RenderWidgetHost* widget = widgets->GetNextHost()) {
      ...
      // see if it's a aviator://memory URL (we don't want to count these in
      // the total memory usage of the browser).
      //
      // When we reach here, aviator://memory will be the pending entry since
      // we haven't responded with any data such that it would be committed.
      // If you have another aviator://memory tab open (which would be
