...
#include "chrome/browser/championconfig/chromeimporter/chrome_profile_lock.h"	//champion for import bookmarks from chrome
...
//const char location[]="D:\\chromium35_work\\src\\utpal.txt";
...
void ExternalProcessImporterHost::StartImportSettings(
  ...
  //Added for champion import bookmarks from chrome
  /*if (!CheckForChromeLock(source_profile)) {
    Cancel();
    return;
  }*/ //block for subpress the warning message utpal 
  //End
...
void ExternalProcessImporterHost::ShowChromeWarningDialog() {
  DCHECK(!headless_);
  importer::ShowImportLockChromeDialog(true,parent_window_,base::Bind(&ExternalProcessImporterHost::OnImportLockDialogEndChrome,weak_ptr_factory_.GetWeakPtr()));
}
...
void ExternalProcessImporterHost::OnImportLockDialogEndChrome(bool is_continue) {
  if (is_continue) {
    // champion : User chose to continue, then we check the lock again to 
    // make sure that Firefox has been closed. Try to import the settings
    // if successful. Otherwise, show a warning dialog.
...
bool ExternalProcessImporterHost::CheckForChromeLock(
 ...
 /* #if defined(OS_WIN)
	std::wstring srcpath=source_profile.source_path.value();
  #elif defined(OS_MACOSX)
	//std::string srcpath=const_cast<std::string>(source_profile.source_path);
  #else
	std::string srcpath=const_cast<std::string>(source_profile.source_path);
  #endif*/
  if (source_profile.source_path.empty()){ // champion: To show chrome in import list even though pref folder is not there. (Navneet)
  //out.open(location,std::ios::app);
  	return true;
  }
  chrome_lock_.reset(new ChromeProfileLock(source_profile.source_path));
  if (chrome_lock_->HasAcquired())
    return true;
