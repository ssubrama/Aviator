...
#include "chrome/browser/championconfig/chromeimporter/chrome_importer_utils.h" //added for champion
...
void ExternalProcessImporterClient::StartProcessOnIOThread(
  ...
  /*Added for chrome import bookmarks for champion*/
  base::EnvironmentMap chromeenv;
  std::string chromedylib_path = GetChromeDylibPath().value();
  if (!chromedylib_path.empty())
    chromeenv["DYLD_FALLBACK_LIBRARY_PATH"] = chromedylib_path;
  utility_process_host_->SetEnv(chromeenv);
  /*End*/
#endif
  ...
  /*Added for importing from chrome browser champion*/
  localized_strings.SetString(
      base::IntToString(IDS_BOOKMARK_GROUP_FROM_CHROME),
  ...
  /*Added for importing from chrome browser champion*/
  localized_strings.SetString(
      base::IntToString(IDS_IMPORT_FROM_CHROME),
