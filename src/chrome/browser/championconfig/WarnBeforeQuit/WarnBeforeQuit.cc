...
#include "chrome/browser/ui/browser_command_controller.h" // champion: Different warn before quit for different window (Navneet)
#include "chrome/browser/ui/browser_iterator.h"           // champion: Different warn before quit for different window (Navneet)
...
void WarnBeforeQuit::WindowClosing() {
  /*if (browser_ && browser_->IsCloseAllBrowser())
    WarnBeforeQuit::closeAllWindow = true;*/
...
void WarnBeforeQuit::ButtonPressed(Button* sender, const ui::Event& event) {
  ...
  if (sender != ok_button_ && sender != cancel_button_) {
    ...
    // champion : To show only one warn before quit for all windows when closing all from wrench menu (Navneet)
    Warnbeforequit2ndWindow(QUIT_ON_CLOSE, this);
    ...
    instance()->GetWidget()->Close();
    // champion: To show only one warn before quit for all windows when closing all from wrench menu (Navneet) start
    if (chrome::BrowserCommandController::quitAll == 1){
      chrome::BrowserCommandController::quitAll = -1;
    ...
    // champion: To show only one warn before quit for all windows when closing all from wrench menu (Navneet) end
  } else if (sender == cancel_button_) {
    // dont close the browser
    WarnBeforeQuit::bQuitAviator = false;

    if (chrome::BrowserCommandController::quitAll == 1){ // champion: To close all windows by one warn before quit dialogue from wrench menu (Navneet)
      chrome::BrowserCommandController::quitAll = 0;
    }
...
//bool WarnBeforeQuit::closeAllWindow = false;

static int ConsolePosX = 400;
static int ConsolePosY = 300;

bool Warnbeforequit(Operation operation, gfx::NativeWindow ptrNativeWnd, Browser* browser, bool& checkStockCType, WarnBeforeQuit *warnBeforeQuit) {
  warnBeforeQuit->instance_ = warnBeforeQuit;
  warnBeforeQuit->browser_ = browser;
  warnBeforeQuit->checkStockCloseType = &checkStockCType;
  warnBeforeQuit->gptrNativeWnd = ptrNativeWnd;
  ...
  Widget::CreateWindowWithParentAndBounds(warnBeforeQuit, ptrNativeWnd, gfx::Rect(ConsolePosX - 150, ConsolePosY - 100, 300, 200))->Show();
...
void Warnbeforequit2ndWindow(Operation operation, WarnBeforeQuit *warnBeforeQuit) {
  // champion : Different warn before quit for different window (Navneet) 
  warnBeforeQuit->gptrParentchild = warnBeforeQuit->instance()->GetWidget()->GetNativeWindow();

  if (WarnBeforeQuit2::instance1())
    WarnBeforeQuit2::instance1()->GetWidget()->Activate();
  else
    // champion: Different warn before quit for different window (Navneet)
    Widget::CreateWindowWithParentAndBounds(new WarnBeforeQuit2(operation),warnBeforeQuit->gptrParentchild,
                                            gfx::Rect(ConsolePosX - 150, ConsolePosY - 115, 350, 230))->Show(); 
}
