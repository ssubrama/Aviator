...
UpdateMechwindow::UpdateMechwindow(Operation operation)
  ...
  Text_label2_= new Label(ASCIIToUTF16("take effect. Remember to save any information you need"));
  Text_label3_= new Label(ASCIIToUTF16("before restarting."));
}
...
void UpdateMechwindow::ButtonPressed(Button* sender, const ui::Event& event) {
  ...
  /*  OSVERSIONINFO osvi;
    ...
    if (atof(WINDOWVERSIONXP)== atof(version.c_str())){
      std::string lock;
      lock.append(getenv("ALLUSERSPROFILE"));
      lock.append("\\");
      lock.append(LOCKFILENAME);
      std::ofstream writer (lock.c_str());
	    if (writer.is_open()){
	 	    writer.close();
      }
    }
