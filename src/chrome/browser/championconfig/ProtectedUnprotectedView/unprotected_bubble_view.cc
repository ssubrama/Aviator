...
#include "chrome/browser/championconfig/protectedunprotectedview/unprotected_promo_view.h" // champion
...
UnProtectedBubbleView* UnProtectedBubbleView::unprotected_bubble_ = NULL;

// static
void UnProtectedBubbleView::ShowBubble(views::View* anchor_view) {
  if (IsShowing())
    return;

  unprotected_bubble_ = new UnProtectedBubbleView(anchor_view);
  views::BubbleDelegateView::CreateBubble(unprotected_bubble_)->Show();
}

// static
bool UnProtectedBubbleView::IsShowing() {
  return unprotected_bubble_ != NULL;
}

void UnProtectedBubbleView::Hide() {
  if (IsShowing()) {
    views::Widget *upWidget = unprotected_bubble_->GetWidget();
    if (upWidget)
      upWidget->Close();
  }
}

UnProtectedBubbleView::~UnProtectedBubbleView() {
}

void UnProtectedBubbleView::WindowClosing() {
  // We have to reset |bubble_| here, not in our destructor, because we'll be
  // destroyed asynchronously and the shown state will be checked before then.
  DCHECK_EQ(unprotected_bubble_, this);
  unprotected_bubble_ = NULL;

}

void UnProtectedBubbleView::Init() {
  views::Label* title_label = new views::Label(l10n_util::GetStringUTF16(IDS_UNPROTECTED_BUBBLE));
  ...
  cs->AddColumn(GridLayout::FILL, GridLayout::FILL, 1, GridLayout::USE_PREF, 0, 0);
  layout->StartRow(0, SYNC_PROMO_COLUMN_SET_ID);

  sync_promo_view_ = new UnProtectedPromoView();
  layout->AddView(sync_promo_view_);

  AddAccelerator(ui::Accelerator(ui::VKEY_RETURN, ui::EF_NONE));
}

UnProtectedBubbleView::UnProtectedBubbleView(views::View* anchor_view ) : BubbleDelegateView (anchor_view, views::BubbleBorder::TOP_RIGHT) {
...
base::string16 UnProtectedBubbleView::GetTitle() {
  return base::string16();
}

gfx::Size UnProtectedBubbleView::GetMinimumSize() const {
