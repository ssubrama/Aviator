...
TEST_F(SupervisedUserURLFilterTest, Basic) {
  ...
  EXPECT_TRUE(IsURLWhitelisted("aviator://youtube.com/"));
  EXPECT_TRUE(IsURLWhitelisted("aviator://extensions/"));
...
TEST_F(SupervisedUserURLFilterTest, HasFilteredScheme) {
  ...
  EXPECT_FALSE(
      SupervisedUserURLFilter::HasFilteredScheme(GURL("aviator://example.com")));
  EXPECT_FALSE(
