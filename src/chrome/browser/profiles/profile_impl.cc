...
void ProfileImpl::RegisterProfilePrefs(
  ...
  registry->RegisterBooleanPref(
      ...
      false,
      user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
  // champion : kHomePage already registered in browser_ui_prefs.cc (Amresh)
  ...
  // Champion artf414382 Build library on Mac	
  // Champion artf360025 ENABLE "Clear cookies and other site data when I quit my browser"								
  if(CLEAR_COOKIES)		
...
GURL ProfileImpl::GetHomePage() {
  ...
  if (GetPrefs()->GetBoolean(prefs::kHomePageIsNewTabPage))
    return GURL(chrome::kChromeUINewUnprotectedWindowURL /*chrome::kChromeUINewTabURL*/); // champion - homepage
  
  if (StartupBrowserCreator::bUnProtectedBlankTabForTaskBarWindow) { // champion_balaji Added for windows version7+ Taskbar open unprotectedmode window should show blank url(new tab url)
    StartupBrowserCreator::bUnProtectedBlankTabForTaskBarWindow = false;
  ...
  GURL home_page(url_fixer::FixupURL(GetPrefs()->GetString(prefs::kHomePage),
                                     std::string()));
  if (!home_page.is_valid())
    return GURL(chrome::kChromeUINewUnprotectedWindowURL); // champion - homepage
  return home_page;
}
