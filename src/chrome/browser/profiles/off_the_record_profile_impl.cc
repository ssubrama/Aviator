...
OffTheRecordProfileImpl::~OffTheRecordProfileImpl() {
  ...
  const char myWindowSize[] = "browser.window_placement"; // Champion : To retain windows size (Navneet)
  const char myToolbarSize[] = "extensions.toolbarsize";  // Champion : To set tool bar size (Navneet)
  GetOriginalProfile()->GetPrefs()->GetDictionary(myWindowSize)->MergeDictionary(prefs_->GetDictionary(myWindowSize)); // Champion : To retain windows size (Navneet)
  GetOriginalProfile()->GetPrefs()->SetInteger(myToolbarSize, prefs_->GetInteger(myToolbarSize)); // Champion : To set tool bar size (Navneet)
...
HostContentSettingsMap* OffTheRecordProfileImpl::GetHostContentSettingsMap() {
  ...
  if (!host_content_settings_map_.get()) {
    // champion : content setting saving fix (amresh)
    //host_content_settings_map_ = new HostContentSettingsMap(GetPrefs(), true);
	host_content_settings_map_ = new HostContentSettingsMap(profile_->GetPrefs(), false);
#if defined(ENABLE_EXTENSIONS)
