...
#if  defined (OS_WIN)
#include "components/policy/core/browser/url_blacklist_manager.h" // champion:  To load connection control rules for the first time at startUp (Navneet)

#define WINDOWVERSION7 "6.1" // champion - Default browser - navneet
#endif
...
MasterPrefs::MasterPrefs()
    : ping_delay(0),
      ...
      // make_chrome_default_for_user(true), // Champion Navneet, changed for win7 to make Aviatopr default by default.
      suppress_first_run_default_browser_prompt(false) {
        // Champion : Default browser - navneet start
        #if defined (OS_WIN)
		OSVERSIONINFO osvi; 
        ...
        if (atof(WINDOWVERSION7)<= atof(version.c_str())){
          make_chrome_default_for_user = true;
        }
        // Champion : Default browser - navneet end
		#endif
}
...
bool IsChromeFirstRun() {
  ...
  // Champion: Added to load connection control rules at first launch (Navneet) //champion dinesh commented
  /*#if defined (OS_WIN)
  if (internal::g_first_run == internal::FIRST_RUN_TRUE){
    //policy::URLBlacklist::championIsFirstRun = true;
    return true;
  }
  else{
   // policy::URLBlacklist::championIsFirstRun = false;
    return false;
  }
  #endif*/
}
...
void AutoImport(
  ...
  if (importer_list->count() > 0) {
    ...
    SetImportItem(user_prefs,
                  ...
                  &items);*/ // champion dont import history and search engine
    SetImportItem(user_prefs,
