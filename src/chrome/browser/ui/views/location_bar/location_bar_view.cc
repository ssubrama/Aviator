...
#include "chrome/browser/championconfig/protectedunprotectedview/protected_view.h"   // champion added for protected/Unprotected view
#include "chrome/browser/championconfig/protectedunprotectedview/unprotected_view.h" // champion added for protected/Unprotected view
...
LocationBarView::LocationBarView(Browser* browser,
    ...
    : LocationBar(profile),
	  ...
	  protected_view_(NULL),       // champion added for protected/Unprotected view
      unprotected_view_(NULL),     // champion added for protected/Unprotected view 
...
void LocationBarView::Init() {
  ...
  // champion added for protected/Unprotected view
  protected_view_ = new ProtectedView(browser_);
  protected_view_->SetVisible(false);
  AddChildView(protected_view_);
  
  unprotected_view_ = new UnProtectedView(browser_);
  unprotected_view_->SetVisible(false);
  AddChildView(unprotected_view_);
  // champion end for protected/Unprotected view
...
void LocationBarView::Layout() {
  ...
  if(!browser_->profile()->IsOffTheRecord()) {
    if (unprotected_view_ && unprotected_view_->visible()) {
	  trailing_decorations.AddDecoration(
      vertical_edge_thickness(), location_height,
      /*GetBuiltInHorizontalPaddingForChildViews(),*/ unprotected_view_);
	} 
  } else {
    if (protected_view_ && protected_view_->visible()) {
	  trailing_decorations.AddDecoration(
      vertical_edge_thickness(), location_height,
      /*GetBuiltInHorizontalPaddingForChildViews(),*/ protected_view_);
	}
  } // champion end for protected/Unprotected view
  ...
  // "Settings" <-> "aviator://settings".  Skip any pre-hostname text.
...
void LocationBarView::Update(const WebContents* contents) {
  ...
  // champion added for protected/Unprotected view	
  if (protected_view_)
    protected_view_->SetVisible(true);
	
  if (unprotected_view_)
    unprotected_view_->SetVisible(true);
  // champion end for protected/Unprotected view
