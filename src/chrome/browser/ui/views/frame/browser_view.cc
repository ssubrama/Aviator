...
#include "chrome/browser/championconfig/protectedunprotectedview/protected_bubble_view.h"   // champion added for protected
#include "chrome/browser/championconfig/protectedunprotectedview/unprotected_bubble_view.h" // champion added for unprotected
...
base::OneShotTimer<BrowserView> protected_timer_;   // champion protected
base::OneShotTimer<BrowserView> unprotected_timer_; // champion unprotected
static bool checkStockCloseType = true; // champion : for WBQ (Amresh)
...
void BrowserView::Close() {
  frame_->Close();
  frame_->FrameClosed = true;

  // champion : reset flags after frame is closed (amresh)
  if (browser_->profile()->IsOffTheRecord())
    options::BrowserOptionsHandler::UserProfileDeleted = false;
  checkStockCloseType = true;
  views::WarnBeforeQuit::closeAll = false;
  BrowserTabStripController::tabclosecheck = false;
  TabStripModel::createshortcutcheck = false;//champion - Dinesh
}
...
void BrowserView::ShowProtectedBubble() { // champion added for protected
  /*if (!protected_timer_.IsRunning()) {
    ProtectedBubbleView::ShowBubble(GetToolbarView()->GetProtectedBubbleAnchor());
    protected_timer_.Start(FROM_HERE,TimeDelta::FromSeconds(4), this,&BrowserView::HideProtectedBubble);
	}*/
  ProtectedBubbleView::ShowBubble(GetToolbarView()->GetProtectedBubbleAnchor());
}

void BrowserView::HideProtectedBubble() { // champion added for protected
	/*if (protected_timer_.IsRunning()) 
		protected_timer_.Stop();*/
  ProtectedBubbleView::Hide();
}

void BrowserView::ShowUnProtectedBubble() { // champion added for protected
  /*if (!unprotected_timer_.IsRunning()) {
    UnProtectedBubbleView::ShowBubble(GetToolbarView()->GetUnProtectedBubbleAnchor());
    unprotected_timer_.Start(FROM_HERE,TimeDelta::FromSeconds(4), this,&BrowserView::HideUnProtectedBubble);
	}*/
  UnProtectedBubbleView::ShowBubble(GetToolbarView()->GetUnProtectedBubbleAnchor());
}

void BrowserView::HideUnProtectedBubble() { // champion added for protected
	/*if (unprotected_timer_.IsRunning())
		unprotected_timer_.Stop();*/
...
bool BrowserView::CanClose() {
  // champion : decide how to quit for protected window (amresh)
  if(!BrowserTabStripController::tabclosecheck) {
    if (!browser_->IsExitStockType() && 
        !frame_->FrameClosed &&
        !views::WarnBeforeQuit::closeAll && 
        !views::WarnBeforeQuit::bQuitAviator &&
        checkStockCloseType) {
      gfx::NativeWindow parent = browser_->window()->GetNativeWindow();
      // champion : Different warn before quit for different window (Navneet)
      views::Warnbeforequit(QUIT_ON_CLOSE, parent, browser_.get(), checkStockCloseType, new views::WarnBeforeQuit(QUIT_ON_CLOSE,ui::MODAL_TYPE_CHILD));
      return false;
    }
  }
...
void BrowserView::InitViews() {
  ...
  // champion : we need to save this pointer for our warn before quit or
  //            else browserview objects may not return tabstrip controller
  //            which is required to close the tab. (amresh)
  /*BrowserTabStripController**/ tabstrip_controller_ =
      new BrowserTabStripController(browser_.get(),
                                    browser_->tab_strip_model());
  tabstrip_ = new TabStrip(tabstrip_controller_);
  top_container_->AddChildView(tabstrip_);
  tabstrip_controller_->InitFromModel(tabstrip_);
