...
#include "chrome/browser/ui/views/frame/browser_view.h"                    // champion - header for WBQ (amresh)
#include "chrome/browser/championconfig/warnbeforequit/warnbeforequit.h"   // champion - header for WBQ (amresh)
...
bool BrowserTabStripController::tabclosecheck=false;
static CloseTabSource gCTS;
static bool checkStockCloseType = true;

// champion : called by warn before quit (amresh)
void BrowserTabStripController::CloseSingleTab() {
  BrowserTabStripController::tabclosecheck = true;
  hover_tab_selector_.CancelTabTransition();
  tabstrip_->PrepareForCloseAt(0, gCTS);
  model_->CloseWebContentsAt(0,
...
void BrowserTabStripController::CloseTab(int model_index,
  ...
  // champion : set pointer and tab index for CloseSingleTab function (amresh)
  gCTS = source;

  // champion : handle single close tab event (amresh)
  if (!browser_->IsExitStockType() && 
    ...
    // champion: To close all windows by one warn before quit dialogue from wrench menu (Navneet)
    views::Warnbeforequit(QUIT_ON_CLOSE, parent, browser_, checkStockCloseType,new views::WarnBeforeQuit(QUIT_ON_CLOSE, ui::MODAL_TYPE_CHILD));
  } else if (!browser_->IsExitStockType() && GetCount() == 1) {
    return;
  } else {
    // champion : default code (amresh)
