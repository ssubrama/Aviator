...
// chmapion : Extension not popping issue fix (amresh)
static int extNeedsDisableAndEnable = 0;
...
void BrowserActionsContainer::RefreshBrowserActionViews() {
  ...
  if (!extNeedsDisableAndEnable)
    service = extensions::ExtensionSystem::Get(profile_)->extension_service();
  for (size_t i = 0; i < browser_action_views_.size(); ++i) {
    if (!extNeedsDisableAndEnable && service) {
      const extensions::Extension* extension = browser_action_views_[i]->GetExtension();
      if (extension) {
        std::string extId = extension->id();
        service->DisableExtension(extId, extensions::Extension::DISABLE_USER_ACTION);
        service->EnableExtension(extId);
      }
    }
    browser_action_views_[i]->button()->UpdateState();
  }
  if (!extNeedsDisableAndEnable) 
    extNeedsDisableAndEnable++;
  // champion : end
}
...
void BrowserActionsContainer::SaveDesiredSizeAndAnimate(
  ...
  /* if (!profile_->IsOffTheRecord())*/ //champion - navneet for addon changes
