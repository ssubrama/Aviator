...
#include "chrome/browser/championconfig/protectedunprotectedview/protected_view.h"   // champion added for protected
#include "chrome/browser/championconfig/protectedunprotectedview/unprotected_view.h" // champion added for protected
...
views::View* ToolbarView::GetProtectedBubbleAnchor() { // champion added for protected
  views::View* protected_view = location_bar()->protected_view();
  if (protected_view && protected_view->visible())
    return protected_view;
  return app_menu_;
}

views::View* ToolbarView::GetUnProtectedBubbleAnchor() { // champion added for Unprotected
  views::View* unprotected_view = location_bar_->unprotected_view();
  if (unprotected_view && unprotected_view->visible())
    return unprotected_view;
  return app_menu_;
}

// champion added for protected/Unprotected
void ToolbarView::OnMouseMoved(const ui::MouseEvent& event) {
	location_bar_->OnMouseMoved (event);
}
