...
class InstalledBubbleContent : public views::View,
  ...
  InstalledBubbleContent(Browser* browser,
    ...
    if (flavors_ & SIGN_IN_PROMO) {
      ...
      sign_in_link_->set_listener(this);
       // AddChildView(sign_in_link_);//champion - Dinesh - For removing Extension bubble sign in link
    }
 ...
 private:
  ...
  int LayoutSigninPromo(int offset_x, int offset_y) {
    ...
    // champion : handled extension installation crash issue (amresh)
    // need to look into it later
    if (sign_in_promo_lines_.size())
