...
// TODO(samarth): delete along with rest of NTP4 code.
// Loads aviator://hang/ into two NTP tabs, ensuring we don't crash.
...
IN_PROC_BROWSER_TEST_F(NewTabUIBrowserTest, DISABLED_ChromeHangInNTP) {
  ...
  // Navigate to aviator://hang/ to stall the process.
  ui_test_utils::NavigateToURLWithDisposition(
      browser(), GURL(content::kChromeUIHangURL), CURRENT_TAB, 0);

  // Visit aviator://hang/ again in another NTP. Don't bother waiting for the
