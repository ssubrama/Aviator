...
WebUIFactoryFunction GetWebUIFactoryFunction(WebUI* web_ui,
  ...
  /* Champion  aviator://feedback for Report an issue*/
  if (url.host() == chrome::kChromeUIFeedbackHost)
    return &NewWebUI<FeedbackUI>;
    
  /* Champion artf414382: Build library on Mac	*/
  /* Champion artf388118: Disable aviator://flags */
  if (url.host() == chrome::kChromeUIFlagsHost) {
    if (DISABLE_CHROME_FLAGS)
      return NULL;                // Disabled in champion
    else
      return &NewWebUI<FlagsUI>;  // Defualt chrome value
  }
  if (url.host() == chrome::kChromeUIFlagsHost)
  ...
  /*if (url.host() == chrome::kChromeUISignInInternalsHost)
    return &NewWebUI<SignInInternalsUI>;*/ //Block for champion
  ...
  // aviator://inspect isn't supported on Android. Page debugging is handled by a
  ...
  if (url.host() == chrome::kChromeUIChromeSigninHost)
    return NULL;//champion - for blocking aviator://aviator-signin Page - Navneet
