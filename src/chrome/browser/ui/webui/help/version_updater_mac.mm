...
#include "content/public/browser/browser_thread.h" // champion

using content::BrowserThread; // champion
...
public:
  AviatorUpdater(VersionUpdaterMac* objVUMac);
  void InvokeUpdate() {
    BrowserThread::PostTask(BrowserThread::FILE, FROM_HERE, base::Bind(
      &AviatorUpdater::UpdateAviatorNew, this));
  }
...
private:
  ...
  void UpdateAviatorNew();
 // void getAviatorRegistryPath();
 // string getUpdatePathFromRegistry();
 // bool CanRequestRestart();
  VersionUpdaterMac *objVerUpdaterMac;
...
AviatorUpdater::AviatorUpdater(VersionUpdaterMac* objVUMac) {
  objVerUpdaterMac = objVUMac;
}

void AviatorUpdater::UpdateAviatorNew() {
  ...
  NSString *ticketPath = [NSString stringWithFormat:@"%@%@", bundlePath,@"/AviatorTicket"];
  
 // GetContentsPath
  
      base::string16 message;
    VersionUpdaterMac::Status status;
    
  try {
  		[t1 setLaunchPath:scriptPath];
  		[t1 setStandardOutput:[NSPipe pipe]];
		  [t1 setArguments:[NSArray arrayWithObjects:@"dryrunticket", @"-store", ticketPath, nil]];		

	  	// Launch (forks) the process
	  	[t1 launch]; // raises an exception if something went wrong
	  	[t1 waitUntilExit];
		
	   	NSData* data = [[[t1 standardOutput] fileHandleForReading] availableData];
      if ((data != nil) && [data length]) {
      NSString* tmpString = [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease];
      NSLog(@"Resultant string : %@", tmpString);
      NSRange isRange = [tmpString rangeOfString:@"com.whitehatsec.aviator" options:NSCaseInsensitiveSearch];
      NSLog(@"Resultant string 1: %d", isRange.location);
      if(isRange.location != NSNotFound) {
         bUpdateAvailable = true;
         status = VersionUpdaterMac::UPDATING;
         objVerUpdaterMac->status_callback_.Run(status, 0, message);
        }
     }
  }
  catch (NSException *exception) {
        // NSLog(@"Problem Running Task - Aviator: %@", [exception description]);
         NSLog(@"Terminationstatus - in catch block: %d", [t1 terminationStatus]);
  } 
    if (bUpdateAvailable) {
         ...
         try {
           [t2 setLaunchPath:scriptPath];
           [t2 setArguments:[NSArray arrayWithObjects:@"runticket", @"-store", ticketPath, nil]];		
      		// Launch (forks) the process
      		[t2 launch]; // raises an exception if something went wrong
      		[t2 waitUntilExit];
        
          status = VersionUpdaterMac::NEARLY_UPDATED; 
          objVerUpdaterMac->status_callback_.Run(status, 0, message);     	
          	
         }
         catch (NSException *exception) {
           // NSLog(@"Problem Running Task - Aviator: %@", [exception description]);
           NSLog(@"Terminationstatus - in catch block: %d", [t2 terminationStatus]);
         }
       }
    else 
       status = VersionUpdaterMac::UPDATED; 
 //   if (!VersionUpdaterMac::status_callback_.is_null()) {
      objVerUpdaterMac->status_callback_.Run(status, 0, message);
 //   VersionUpdaterMac::status_callback_.Run(status, 0, message);
   // }
...
// champion - modified for update mechanism in about page
void VersionUpdaterMac::CheckForUpdate(
 ...
 // bool updateAvialable = false;
  
        // champion : create threader weak ptr and invoke updater to do job (Amresh)
 update_launcher_ = new AviatorUpdater(this);
  update_launcher_->InvokeUpdate();
 // updateAvialable = UpdateCheck(); // checking for update 
  

}

bool VersionUpdaterMac::UpdateCheck(){
...
void VersionUpdaterMac::GetUpdate() {
...
// champion -  commented for update mechanism in about page
/*
void VersionUpdaterMac::CheckForUpdate(
    const StatusCallback& status_callback,
    const PromoteCallback& promote_callback) {
...
void VersionUpdaterMac::RelaunchBrowser() const {
  ...
  brestartAviatorToTakeUpdate=true;
