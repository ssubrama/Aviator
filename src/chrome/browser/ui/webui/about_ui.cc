...
#include "chrome/browser/championconfig/varsconfig.h"  //Added for Champion_bookmark
...
#if defined(OS_CHROMEOS)
// aviator://terms falls back to offline page after kOnlineTermsTimeoutSec.
...
std::string ChromeURLs() {
  std::string html;
  std::string strBrowserName(BROWSER_NAME); // champion added
  std::string strCompanyName(COMPANY_NAME); // champion added
  strBrowserName+=" URLS";
  std::string strCompanyBrowserName;
  strCompanyBrowserName=strCompanyName+" "+strBrowserName;				
  //AppendHeader(&html, 0, "Aviator URLs"); // champion blocked
  AppendHeader(&html, 0, strBrowserName);
  AppendBody(&html);
  // html += "<h2>List of WhiteHat Aviator URLs</h2>\n<ul>\n"; // champion blocked
  html += "<h2>List of "+strCompanyBrowserName+"</h2>\n<ul>\n";
  ...
  for (std::vector<std::string>::const_iterator i = hosts.begin();
       i != hosts.end(); ++i)
    //Added for discarding print ipc apps from aviator internal url list champion
    if(*i != "print" && *i != "ipc" && *i != "apps" )  
       html += "<li><a href='aviator://" + *i + "/'>aviator://" + *i + "</a></li>\n";
  html += "</ul>\n<h2>For Debug</h2>\n"
...
bool AboutUIHTMLSource::ShouldDenyXFrameOptions() const {
  ...
  if (source_name_ == chrome::kChromeUITermsHost) {
    // aviator://terms page is embedded in iframe to aviator://oobe.
    return false;
  }
#endif
