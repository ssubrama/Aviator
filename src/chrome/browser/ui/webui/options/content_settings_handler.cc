...
#include <map>
#include <vector>
#include <algorithm> //Champion purge_referrer
#include "base/bind.h"
...
#include "chrome/browser/championconfig/addreferrer/referrer_header_handler.h"//Champion purge_referrer
...
const char kExceptionsLearnMoreUrl[] = AVIATOR_HELP_URL; // champion balaji
...
#endif
  {CONTENT_SETTINGS_TYPE_REFERRER, "referrer"}, // champion : add referrer
};
...
void ContentSettingsHandler::RemoveExceptionFromHostContentSettingsMap(
  ...
  //Added for purge ruffer champion 
  std::string type_string;
  ReferrerHeaderHandler obj;
	std::vector<std::string> ::iterator myiterator;
  std::vector<std::string> ::iterator myiterator1;
  	
  if( type_string == "referrer" )
  {
	   if(obj.Allways_Or_Never)
	  {
	  	net::HttpRequestHeaders::RemoveException(2, pattern);
		/*  for(myiterator=net::HttpRequestHeaders :: BlockVector.begin();myiterator!=net :: HttpRequestHeaders :: BlockVector.end();++myiterator)
		  {
		  	
			  std::string temp(*myiterator);
			  if(temp == pattern )
			  {
				  myiterator1=myiterator;
				  net ::HttpRequestHeaders :: BlockVector.erase(myiterator1);
				  break;
			  }
			  else
				  continue;
	  ...
	  {
	  	net::HttpRequestHeaders::RemoveException(1, pattern);
	/*	  for(myiterator=net::HttpRequestHeaders::AllowVector.begin();myiterator!=net::HttpRequestHeaders::AllowVector.end();++myiterator)
		  {
			  std::string temp(*myiterator);
			  if(temp == pattern )
			  {
				  myiterator1=myiterator;
				  net ::HttpRequestHeaders :: AllowVector.erase(myiterator1);
				  break;
			  }
			  else
				  continue;
...
//Added for purge referrer for removing element from vector
//template<class T>
void RemoveElement(std ::vector<std :: string >&vec,std ::string& value)
{
  //Find new end iterator
  std::vector<std :: string>::iterator newEnd = std::remove(vec.begin(), vec.end(), value);
...
void ContentSettingsHandler::SetException(const base::ListValue* args) {
  ...
  CHECK(args->GetString(3, &setting));
   ...
   //Added for purge ruffer 

  if ( mode == "import")
  {
	   if(setting == "allow") 
	 	 		 net::HttpRequestHeaders::Insert(1, pattern); //.push_back(pattern);
		else
	 			 net::HttpRequestHeaders::Insert(2, pattern); //.push_back(pattern);
  ...
  {
  	if(setting == "block" )
		  net::HttpRequestHeaders::Insert(2, pattern); 	
	  if(setting == "allow" )
			net::HttpRequestHeaders::Insert(1, pattern); 
  } */ // champion_23
