...
#include "chrome/browser/championconfig/addreferrer/referrer_header_handler.h" //Added for champion
#include "chrome/browser/championconfig/addreferrer/referrer_header_exception_handler.h" //Added for champion
#include "chrome/browser/championconfig/connectioncontrol/connection_control_handler.h" //Added for champion NPCC
...
OptionsUI::OptionsUI(content::WebUI* web_ui)
  ...
  /*Added for referrer header for champion */
  AddOptionsPageUIHandler(localized_strings, new ReferrerHeaderHandler());
  /*Added for connection control for champion */
  AddOptionsPageUIHandler(localized_strings, new ConnectionControlHandler());
#if defined(OS_CHROMEOS)
  AddOptionsPageUIHandler(localized_strings,
  ...
  // Set up the aviator://settings-frame/ source.
...
#if defined(OS_CHROMEOS)
  // Set up the aviator://userimage/ source.
...
void OptionsUI::InitializeHandlers() {
  Profile* profile = Profile::FromWebUI(web_ui());
  // champion : this dcheck not required as we open option in incognito mode (Amresh)
