...
#include "chrome/browser/profiles/profile.h" // champion navneet 
#include "chrome/common/pref_names.h" // champion navneet
#include "chrome/browser/championconfig/varsconfig.h" // champion navneet
#include "base/prefs/pref_service.h" //champion
...
void SearchEngineManagerHandler::OnModelChanged() {
  ...
  // champion navneet update homepage URL once the default search engine is changed
  std::string disconnectURl =list_controller_->url_model()->profile()->GetPrefs()->GetString(prefs::kHomePage);
  ...
  if(disconnectURl == HOMEPAGE_URL || 
     ...
     disconnectURl == HOMEPAGE_URL"#searchEngine=DuckDuckGo" ||  
	   ...
	   disconnectURl == HOMEPAGE_URL"#searchEngine=yahoo" || 
     disconnectURl == HOMEPAGE_URL"#searchEngine=duckduckgo") {
    //std::string defaultSearchEngine = (list_controller_->url_model()->profile()->GetPrefs()->GetString(prefs::kDefaultSearchProviderName));
	  base::string16 defaultSearchEngine = default_engine->short_name();
    if (defaultSearchEngine.compare(base::ASCIIToUTF16("Yahoo! (Disconnect)")) == 0 )
      loadUrl.append("#searchEngine=Yahoo");
	  else if (defaultSearchEngine.compare(base::ASCIIToUTF16("DuckDuckGo (Disconnect)")) == 0 )
      loadUrl.append("#searchEngine=DuckDuckGo");
	  else if (defaultSearchEngine.compare(base::ASCIIToUTF16("Bing (Disconnect)")) == 0 )
      loadUrl.append("#searchEngine=Bing");
	  else if (defaultSearchEngine.compare(base::ASCIIToUTF16("Google (Disconnect)")) == 0 )
      loadUrl.append("#searchEngine=Google");
    list_controller_->url_model()->profile()->GetPrefs()->SetString(prefs::kHomePage, loadUrl);
  }
  // champion navneet update homepage URL once the default search engine is changed End
