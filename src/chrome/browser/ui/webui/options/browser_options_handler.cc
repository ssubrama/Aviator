...
void BrowserOptionsHandler::GetLocalizedValues(base::DictionaryValue* values) {
  ...
  static OptionsStringResource resources[] = {
    // champion Added for Add referrer & warn_before_quit i18n name string CHAMPION_BALAJI START	
    { "referrerHeaderPage",             IDS_OPTIONS_REFERRER_HEADER_TITLE },
    ...
    { "sectionTitleQuit",               IDS_OPTIONS_QUIT_GROUP_NAME},
    { "warnBeforeQuitEnabled",          IDS_OPTIONS_QUIT_ENABLE_QUIT},

    { "sectionTitleNotification",       IDS_OPTIONS_NOTIFICATION_GROUP_NAME}, // champion - notification (navneet)
    { "notificationKeychainEnabled",    IDS_OPTIONS_NOTIFICATION_KEYCHAIN}, // champion : keycahin (navneet) 
  
    { "popForDefaultSearch",            IDS_OPTIONS_DEFAULT_SEARCH_POPUP}, // champion : Disconnect search popup ( navneet )
	  
	// champion Added for Add referrer & warn_before_quit i18n name string CHAMPION_BALAJI END	
	...
	{ "passwordManagerIncognitoEnabled", IDS_OPTIONS_PASSWORD_MANAGER_INCOGNITO_ENABLE },// added champion_pwd
//    { "passwordGenerationEnabledDescription",
 //     IDS_OPTIONS_PASSWORD_GENERATION_ENABLED_LABEL },
	...
	{ "privacyAddReferrerButton",                // champion
      IDS_OPTIONS_PRIVACY_ADD_REFERRER_BUTTON },
...
void BrowserOptionsHandler::RegisterMessages() {
  ...
  // champion added for warn_before_quit
   web_ui()->RegisterMessageCallback(
      "enableWarningDialogWhileQuit",
      base::Bind(&BrowserOptionsHandler::EnableWarningDialogWhileQuit,
                 base::Unretained(this)));
#if defined(OS_MACOSX)
      web_ui()->RegisterMessageCallback(
      "notificationKeyChainEnabled",
      base::Bind(&BrowserOptionsHandler::notificationKeyChainEnabled,
                 base::Unretained(this)));      
...
// champion added for warn_before_quit
void BrowserOptionsHandler::EnableWarningDialogWhileQuit(const base::ListValue* args) {
  ...
  // arguably wants to be notified when that changes.
  PrefService* prefs = Profile::FromWebUI(web_ui())->GetPrefs();
  prefs->SetBoolean(prefs::kWarnBeforeQuitEnabled, enable);
}
#if defined(OS_MACOSX)
 void BrowserOptionsHandler::notificationKeyChainEnabled(const base::ListValue* args) {
  ...
  // arguably wants to be notified when that changes.
  PrefService* prefs = Profile::FromWebUI(web_ui())->GetPrefs();
  prefs->SetBoolean(prefs::kNotificationKeyChainAccess, enable);
...
//static 
#if defined(OS_WIN) 
bool BrowserOptionsHandler::UserProfileDeleted = false; // champion : no quit dialog on user profile deletion (Amresh)
#endif
#if defined(OS_MACOSX) 
bool options::BrowserOptionsHandler::bUserProfileDeleted=false; // champion Balaji UserProfileDeleted
#endif
void BrowserOptionsHandler::DeleteProfile(const base::ListValue* args) {
  ...
  if (!base::GetValueAsFilePath(*file_path_value, &file_path))
    return;
  #if defined(OS_WIN)
  options::BrowserOptionsHandler::UserProfileDeleted = true;  
  #endif
  #if defined(OS_MACOSX)
  options::BrowserOptionsHandler::bUserProfileDeleted=true; 
  #endif
  helper::DeleteProfileAtPath(file_path, web_ui());  
  //options::BrowserOptionsHandler::bUserProfileDeleted=false; 
}
...
BrowserOptionsHandler::GetSyncStateDictionary() {
  ...
  // champion : as we are not allowing signin in setting page, block it (amresh)
...
void BrowserOptionsHandler::UpdateSyncState() {
  ...
  #if defined(OS_MACOSX)
  PrefService* pref_service = Profile::FromWebUI(web_ui())->GetPrefs();
  base::FundamentalValue notification_enabled(pref_service->GetBoolean(prefs::kNotificationKeyChainAccess));          
    web_ui()->CallJavascriptFunction("BrowserOptions.notificationKeyChainEnabledJS",
                              notification_enabled); //champion
  #endif  // defined(OS_MACOSX)                
}
