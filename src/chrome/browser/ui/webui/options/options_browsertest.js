...
OptionsWebUITest.prototype = {
   ...
   */
  browsePreload: 'aviator://settings-frame',
...
TEST_F('OptionsWebUITest', 'DISABLED_testRefreshStaysOnCurrentPage',
  ...
  assertEquals('aviator://settings-frame/searchEngines', document.location.href);
  assertFalse($('search-engine-manager-page').hidden);
  testDone();
});
...
OptionsWebUIExtendedTest.prototype = {
   ...
   *     will be used. In either case, 'aviator://settings-frame/' will be
   *     prepended.
   * @private
   */
  verifyOpenPages_: function(expectedPages, opt_expectedUrl) {
    ...
    // TODO(pamg): Add a test fixture that loads aviator://settings and uses
    ...
    var fullExpectedUrl = 'aviator://settings-frame/' + expectedUrl;
   ...
   *     current page as the last entry. The base URL (aviator://settings-frame/)
    ...
    OptionsWebUIExtendedTest.verifyHistoryCallback = function(results) {
      ...
      for (var i = 0; i < expectedHistory.length; ++i) {
        fullExpectedHistory.push(
            'aviator://settings-frame/' + expectedHistory[i]);
      }
...
OptionsWebUIRedirectTest.prototype = {
  ...
  /** @override */
  browsePreload: 'aviator://settings-frame/nonexistantPage',
};

TEST_F('OptionsWebUIRedirectTest', 'TestURL', function() {
  assertEquals('aviator://settings-frame/', document.location.href);
  this.verifyHistory_([''], testDone);
});
