...
#include "components/url_fixer/url_fixer.h"                      // champion : auto focus on addressbar (balaji)
...
// Currently this applies to some aviator:// pages which we always want to open
...
bool AdjustNavigateParamsForURL(chrome::NavigateParams* params) {
  // champion : some internal urls to load in protected mode (amresh)
  if (params->target_contents != NULL ||
    params->url.spec().find ("aviator://") != std::string::npos ||
  ...
  if (profile->IsOffTheRecord() || params->disposition == OFF_THE_RECORD) {
    ...
    // champion - commented since it is not opening the preference page
...
void Navigate(NavigateParams* params) {
  ...
  if (params->disposition != CURRENT_TAB) {
   ...
   #if defined(OS_WIN)
   GURL home_page(url_fixer::FixupURL(params->browser->profile()->GetPrefs()->GetString(prefs::kHomePage),std::string()));
   if(params->url.spec()==""/*home_page.spec()*/) {
      BrowserView* browser_view = BrowserView::GetBrowserViewForBrowser(params->browser);
      if (browser_view) {
...
bool IsURLAllowedInIncognito(const GURL& url,
  ...
  // aviator://extensions is on the list because it redirects to
  // aviator://settings.
