...
#include "chrome/browser/championconfig/feedbackui/feedback_ui.h" // champion added for feedback
#include "chrome/browser/championconfig/varsconfig.h" //  champion : To add disconnect homepage on pressing home button. (Navneet)
#if defined(OS_WIN)
#include "chrome/browser/ui/views/frame/browser_view.h"             // champion : auto focus on addressbar (balaji)
...
Browser* OpenEmptyWindow(Profile* profile, HostDesktopType desktop_type) {
  ...
  browser->window()->SetFocusToLocationBar(true);//champion - Dinesh
  #endif
#if defined(OS_WIN)
...
void Home(Browser* browser, WindowOpenDisposition disposition)  { //champion : Reload - Navneet
  //void Home(Browser* browser, WindowOpenDisposition disposition, bool callForReload){
  ...
  /*GURL url;  // champion added for reload : Navneet
  if (callForReload)
    url = const_cast<GURL&> (browser->tab_strip_model()->GetActiveWebContents()->GetURL());
  else
    url = browser->profile()->GetHomePage(); */
  ...
  //  champion : To add disconnect homepage on pressing home button. (Navneet)
 /* std::string disconnectURl = params.url.spec();
  ...
  if(disconnectURl == HOMEPAGE_URL ||
    ...
    std::string defaultSearchEngine = (browser->profile()->GetPrefs()->GetString(prefs::kDefaultSearchProviderName));
...
void NewTab(Browser* browser) {
  ...
  //champion - Start : Location bar Focus - Dinesh
  #if defined(OS_WIN)
  BrowserView* browser_view = BrowserView::GetBrowserViewForBrowser(browser);
  if (browser_view) {
    LocationBarView* location_bar = browser_view->GetLocationBarView();
...
void OpenFeedbackDialog(Browser* browser) {
  ...
  chromefeedback::ShowFeedbackPage1(browser, std::string(), std::string()); // champion feedback
}
