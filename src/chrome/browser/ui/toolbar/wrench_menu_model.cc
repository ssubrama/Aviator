...
bool WrenchMenuModel::IsCommandIdChecked(int command_id) const {
  ...
  if(!WANT_BOOKMARK_BAR && command_id == IDC_SHOW_BOOKMARK_BAR) {
...
void WrenchMenuModel::Build(bool is_new_menu) {
  ...
  if(!CHAMPION_INTERNAL_BROWSER)    // champion
    AddItemWithStringId(IDC_NEW_WINDOW, IDS_NEW_WINDOW);

  if (!CHAMPION_INTERNAL_BROWSER && ShouldShowNewIncognitoWindowMenuItem()) // champion 
  ...
  //Champion artf360022	Single site browser    
  if(!SINGLE_SITE||SINGLE_SITE||!CHAMPION_INTERNAL_BROWSER) { //artf381078 : To disable Bookmarks from wrench menu
...
//champion : win 8 relaunch menu removed (amresh)
/*#if defined(OS_WIN)
...
//champion : ends
  ...
  /* champion No "Sign in to Chromium..."
#if !defined(OS_CHROMEOS)
