...
#if defined(OS_WIN)
#include "chrome/browser/championconfig/warnbeforequit/WarnBeforeQuit.h" // champion : warn_before_quit (Amresh)
#endif
#include "chrome/browser/ui/webui/options/browser_options_handler.h" // champion : no dialog when quit after delete profile (Amresh)
#include "content/browser/download/download_item_impl.h"  // champion: Changes for downlaod crash (Navneet)
#include "chrome/browser/ui/browser_command_controller.h" // champion: Changes for one warn before quit dialogue for close all from wrench menu (Navneet)
#include "chrome/browser/ui/browser_list.h" // champion  navneet 
...
///////////////////////////////////////////////////////////////////////////////
bool bCloseSingleTabAndWindow=false; // champion WBQ balaji
bool bDIPYesClicked=false; // champion Balaji WBQ -DOWNLOAD IN PROFRESS yes clicked
bool bDIPNoClicked=false; // champion Balaji WBQ -DOWNLOAD IN PROFRESS yes clicked
...
WebContents* Browser::OpenURLFromTab(WebContents* source,
  ...
  /* champion -  Email Link feature */ // champion artf831888 : Links (including links in mail) need to be opened in Protected mode
  if((!profile_->IsOffTheRecord()) && (!HomeButtonClicked)/*For not consider Home Button click*/ && 
     (nav_params.disposition == CURRENT_TAB) && (nav_params.transition !=2)/*For not consider Most visited page click*/ &&
     (!nav_params.is_renderer_initiated)/*For not consider internal URl*/) {  	    
    tab_strip_model_->CloseWebContentsAt(tab_strip_model_->active_index(),TabStripModel::CLOSE_NONE);
  	chrome::OpenURLOffTheRecord(profile(), GURL(params.url),chrome::GetActiveDesktop());  	      	
  } else chrome::Navigate(&nav_params);
...
void Browser::NavigateOnThumbnailClick(const GURL& url,
  ...
  // privileged destinations (e.g. aviator://URLs)).
  //
...
// champion : check if exit is chrome type (amresh)
bool Browser::IsExitStockType() const {
  // champion : no dialog when quit after delete profile (Amresh)
  #if defined (OS_WIN)
  if(options::BrowserOptionsHandler::UserProfileDeleted)
    return true; 
  // champion: Changes for one warn before quit dialogue for close all from wrench menu (Navneet)
  if(TabStripModel::createshortcutcheck || chrome::BrowserCommandController::quitAll == -1 ) 
    return true;
  // champion: Changes for downlaod crash and no warn before quit dialogue for close all from wrench menu (Navneet) start
  if (content::DownloadItemImpl::championDownloadCount != 0 ){ 
    // Clicked on Close cancelling download so no need to check for number of windows as all have to be closed
    if (content::DownloadItemImpl::championDownloadCount == -1 ) 
      return true;
    int windowCount = 0;
    int otrWindowCount = 0; 
    /*for (scoped_ptr<chrome::BrowserIterator> it_ptr(
         new chrome::BrowserIterator());
         !it_ptr->done();) {
         it_ptr->Next();
         windowCount++;
    }*/ //champion - navneet - to cancel download at the time of closing profile.
    BrowserList::BrowserVector browsers_to_close;
    for (chrome::BrowserIterator it; !it.done(); it.Next()) {
      if (it->profile()->GetOriginalProfile() == profile_->GetOriginalProfile()){
        browsers_to_close.push_back(*it);
        if (it->profile()->IsOffTheRecord()){ // Find number of protected windows.
          otrWindowCount++;
        }
      }
    }
    for (BrowserList::BrowserVector::const_iterator it = browsers_to_close.begin();
       it != browsers_to_close.end(); ++it) {
        windowCount++;
    }//champion navneet - end
    if (windowCount <= 1)
      return true;
    if (otrWindowCount == 1) // Don't show warn before quit if only one protected window is there. Show download dialogue instead.
      return true;
  }
 #endif

  // champion: Changes for downlaod crash and no warn before quit dialogue for close all from wrench menu (Navneet) end
  if (profile_->IsOffTheRecord() &&
      type() != TYPE_POPUP &&
      profile_->GetPrefs()->GetBoolean("profile.warn_before_quit_enabled"))
    return false;

  return true;
}

// champion : check if multiple browsers are trying to close (amresh)
bool Browser::IsCloseAllBrowser() const {
  int nCloseCount = 0;

  BrowserList* browserList = BrowserList::GetInstance(chrome::HOST_DESKTOP_TYPE_NATIVE);
  if (browserList && browserList->size() < 2)
    return false;

  int nBrowserCount = browserList->size();
  for (int i = 0; i < nBrowserCount; i++) {
    Browser* browser = browserList->get(i);
    if (browser->type() == TYPE_POPUP)
      continue;
    if (!browser->IsAttemptingToCloseBrowser() || nCloseCount > 1)
      break;
    else
      nCloseCount++;
  }

  return (nCloseCount > 1) ? true : false;
}

void Browser::CloseFrame() {
  bCloseSingleTabAndWindow=true;  
  window_->Close();
  bCloseSingleTabAndWindow=false;
}
