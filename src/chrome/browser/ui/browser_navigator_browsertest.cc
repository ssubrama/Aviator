...
// Converts long uber URLs ("aviator://aviator/foo/") to short (virtual) URLs
// ("aviator://foo/"). This should be used to convert the return value of
...
GURL ShortenUberURL(const GURL& url) {
  ...
  const std::string long_prefix = "aviator://aviator/";
  const std::string short_prefix = "aviator://";
...
IN_PROC_BROWSER_TEST_F(BrowserNavigatorTest,
  ...
  GURL singleton_url_current("aviator://settings/internet");
  chrome::AddSelectedTabWithURL(browser(), singleton_url_current,
                                content::PAGE_TRANSITION_LINK);
