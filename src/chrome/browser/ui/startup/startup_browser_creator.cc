...
#if defined(OS_WIN)
#include "chrome/browser/championconfig/RestartUpdate/RestartforUpdate.h" //champion Dinesh for manualrestart
#endif
...
//static
bool StartupBrowserCreator::bUnProtectedBlankTabForTaskBarWindow=false; // champion_balaji   Added for windows version7+ Taskbar open unprotectedmode window should show blank url(new tab url)
...
bool StartupBrowserCreator::LaunchBrowser( 
  ...
  if (!IncognitoModePrefs::ShouldLaunchIncognito(command_line,
                                                profile->GetPrefs())) { // champion_balaji made function changes with negation(!) for taskbar menu window open properly
  ...
  if (IncognitoModePrefs::ShouldLaunchIncognito(command_line,
    ...
    //bUnProtectedBlankTabForTaskBarWindow=true; //champion - Taskbar right click - unprotected mode - Dinesh// champion_balaji   Added for windows version7+ Taskbar open unprotectedmode window should show blank url(new tab url)
    LOG(WARNING) << "Incognito mode disabled by policy, launching a normal "
                 << "browser session.";
  }  // champion_balaji task_bar
...
SessionStartupPref StartupBrowserCreator::GetSessionStartupPref(
  ...
  if (is_first_run && SessionStartupPref::TypeIsDefault(prefs)) {
    ...
    profile->GetPrefs()->SetBoolean(prefs::kDefaultSearchPopUp,false);// champion : To block Disconnect search popUp for first time users - Navneet
  }
...
/*#if defined (OS_MACOSX) 
      if (prefs->GetBoolean(prefs::kAviatorRestarted)) {
       //  champion fix for update mechanism restart dialog to launch in protected mode
       prefs->SetBoolean(prefs::kAviatorRestarted, false);
        pref.type = SessionStartupPref::DEFAULT;
      }
      else 
      ...
      pref.type = SessionStartupPref::LAST;
#if defined(OS_WIN)
      if (prefs->GetBoolean(prefs::kRestartManual))//champion for manual Restart
