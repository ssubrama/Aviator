...
#if defined(OS_WIN)
#include "chrome/browser/ui/views/frame/browser_view.h"  //  champion navneet 
#endif
#include "chrome/browser/ui/views/location_bar/location_bar_view.h" // champion navneet 
...
Browser* StartupBrowserCreatorImpl::OpenTabsInBrowser(
  ...
  if (!browser_creator_ || browser_creator_->show_main_browser_window()){
    browser->window()->Show();
  // champion - homepage Focus - navneet 
  #if defined (OS_WIN)
  ...
  // champion - homepage Focus - navneet
  #if defined (OS_MACOSX)
   browser->window()->SetFocusToLocationBar(true);//champion
  #endif
  }
...
void StartupBrowserCreatorImpl::AddInfoBarsIfNecessary(
  ...
  if (is_process_startup == chrome::startup::IS_PROCESS_STARTUP &&
	...
	// champion blocked for to Remove WhiteHat API Keys are missing Infobar balaji
...
void StartupBrowserCreatorImpl::AddStartupURLs(
  ...
  if (startup_urls->empty()) {
    // champion : to open home page with disconnect url at startup (navneet)
    startup_urls->push_back(GURL(profile_->GetPrefs()->GetString(prefs::kHomePage)));
