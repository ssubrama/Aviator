...
#include "chrome/browser/championconfig/varsconfig.h" // Champion artf414382 Build library on Mac	 
...
#import "chrome/browser/app_controller_mac.h"                                   // Champion Added for WBQ (Balaji)
#include "chrome/browser/download/download_service.h" // champion : WBQ - Download is in progress warning dialog  (Balaji) 25July14
#include "base/strings/string_number_conversions.h" // champion : WBQ - Download is in progress warning dialog  (Balaji) 25July14
#include "chrome/browser/ui/browser_finder.h" // champion : WBQ - Download is in progress warning dialog  (Balaji) 25July14
#include "chrome/browser/ui/webui/options/browser_options_handler.h" // champion Balaji UserProfileDeleted
#include "chrome/browser/browser_shutdown.h" // champion navneet test
#include "chrome/browser/ui/chrome_pages.h" // champion navneet test


bool isShowUnprotectedBubble; // champion_unprotected
bool isShowProtectedBubble;  //  champion_protected added
...
- (id)initWithBrowser:(Browser*)browser takeOwnership:(BOOL)ownIt {
  ...
  if ((self = [super initTabWindowControllerWithTabStrip:hasTabStrip])) {
    ...
    if(!SINGLE_SITE || WANT_WRENCH_MENU )
    {
      // Puts the incognito badge on the window frame, if necessary.
    [self installAvatar];
    }
...
- (BOOL)windowShouldClose:(id)sender {
   // Champion Added for WBQ Start (Balaji)
   if(!options::BrowserOptionsHandler::bUserProfileDeleted) // champion Balaji UserProfileDeleted
  {
     int downloadCount=0;
     downloadCount=DownloadService::NonMaliciousDownloadCountIncognitoProfiles();
     //NSLog(@" In Download start %d",downloadCount);
 if( !browser_->IsExitStockType() && downloadCount > 0 &&
    (chrome::GetTotalBrowserCountForProfile(browser_->profile())==1) /*&& !bDIPYesClicked  && !bDownloadInProgressShowed */) {
   /* NSLog(@" In Download start %d",!bQuitDockAndMenu );
    NSLog(@" In Download start %d",!bCloseSingleTabAndWindow);
    NSLog(@" In Download start %d",!bDownloadInProgressShowed);*/
    if(!bDIPYesClicked /*&& bDIPNoClicked*/) {
     if([self userWillWaitForInProgressDownloadsChampion:downloadCount]){
       chrome::ShowDownloads(browser_.get()); // champion navneet - Download dialog
       browser_shutdown::SetTryingToQuit(false); // champion navneet test
       return NO;
    } 
}
 } 
 else if( !browser_->IsExitStockType() &&
          !bCloseSingleTabAndWindow  && 
          !bQuitDockAndMenu && 
          !bDownloadInProgressShowed) {
          //  int choice = NSRunAlertPanel(@"windowShouldClose", @"else if( !browser_->IsExitStockType() &&",nil, nil, nil);
       //   choice++; 
    	      QuitDialog*	modalDialog		;
     		  modalDialog	= nil ;
              NSString*		imagePath		;
	          imagePath=nil;
	  		  if ( !modalDialog ) {
        		modalDialog = [QuitDialog sharedMySingleton] ;
	    		if (modalDialog) {
	 ...
	 // bCloseSingleTabAndWindow=true; 	
  }   // champion blocked for Download in progress // if( !browser_->IsExitStockType() &&  !bCloseSingleTabAndWindow )
 // } // champion WBQ -DOWNLOAD IN PROFRESS yes clicked
 // Champion Added for WBQ END (Balaji)	 
  } // champion Balaji UserProfileDeleted
...
// champion : WBQ - Download is in progress warning dialog  (Balaji) 25July14
// Helper function for populating and displaying the in progress downloads at
// exit alert panel.
- (BOOL)userWillWaitForInProgressDownloadsChampion:(int)downloadCount {
  ...
  if (downloadCount == 1) {
    ...
    explanationText = l10n_util::GetNSString(IDS_SINGLE_DOWNLOAD_REMOVE_CONFIRM_EXPLANATION); /*@"Download is in progress. Losing Protected mode profile will cancel the download.";*/
  } else {
    // Dialog text: warning and explanation.
    titleText = l10n_util::GetNSStringF(
        IDS_MULTIPLE_DOWNLOADS_REMOVE_CONFIRM_TITLE,
        base::IntToString16(downloadCount));
    explanationText = l10n_util::GetNSString(IDS_MULTIPLE_DOWNLOADS_REMOVE_CONFIRM_EXPLANATION); /*@"Downloads are in progress. Losing Protected mode profile will cancel the downloads.";*/
  }
  ...
  bDownloadInProgressShowed=true; // champion Balaji_28072014
  // 'waitButton' is the default choice.
  int choice = NSRunAlertPanel(titleText, @"%@",waitTitle, exitTitle, nil, explanationText);
 // if (choice==1)
  //  bDIPYesClicked = true;
...
- (NSPoint)unprotectedBubblePoint { // champion_unprotected
  return [toolbarController_ unprotectedBubblePoint];
}
- (NSPoint)protectedBubblePoint { // champion_protected added
  return [toolbarController_ protectedBubblePoint];
}
...
- (void)showUnprotectedBubble
{
    unprotectedBubbleController_=[[UnprotectedBubbleController alloc] initWithParentWindow:[self window]];
    [unprotectedBubbleController_ showWindow:self];
	 timerUnprotected = [NSTimer scheduledTimerWithTimeInterval:5
												  target:self 
												selector:@selector(updateTimerUnprotectedBubble:) 
												userInfo:nil 
												 repeats:NO];	
}
- (void) updateTimerUnprotectedBubble:(NSTimer *) timer {
    	[self closeUnprotectedBubble];
}
- (void)closeUnprotectedBubble
{
   if(isShowUnprotectedBubble)
	{   if([ timerUnprotected isValid ])
	    {
	     [timerUnprotected invalidate];
	     timerUnprotected = nil; 
	     }          
    	isShowUnprotectedBubble=false;
    	if(unprotectedBubbleController_) {
		[unprotectedBubbleController_ close];
		[unprotectedBubbleController_ release];
    	unprotectedBubbleController_ = nil;
    	}
    }
}
- (void)showProtectedBubble // champion_protected added
{
    protectedBubbleController_=[[ProtectedBubbleController alloc] initWithParentWindow:[self window]];
    [protectedBubbleController_ showWindow:self];
    timerProtected = [NSTimer scheduledTimerWithTimeInterval:5
												  target:self 
												selector:@selector(updateTimerProtectedBubble:) 
												userInfo:nil 
												 repeats:NO];	
}
- (void) updateTimerProtectedBubble:(NSTimer *) timer { // champion_protected added
    	[self closeProtectedBubble];
}
- (void)closeProtectedBubble // champion_protected added
{
     if(isShowProtectedBubble)
	{     
	  if ([ timerProtected isValid] ){
     	[timerProtected invalidate];
	    timerProtected = nil;           
      }
      isShowProtectedBubble=false;
      if (protectedBubbleController_) {
	  [protectedBubbleController_ close];
	  [protectedBubbleController_ release];
	   protectedBubbleController_ = nil;
	   }
	}
/*	WebContents* web_contents = nil;
