...
- (BOOL)setAppsPageShortcutButtonVisibility {
  if (!appsPageShortcutButton_.get())
    return NO;
// champion commented the following line to hide apps icon in bookmark bar.
 /* BOOL visible = bookmarkModel_->loaded() &&
  ...
  return visible; */
  // champion added the following line to hide apps icon in bookmark bar.
  [appsPageShortcutButton_ setHidden:true]; // champion added this line to hide the apps icon
  return false;
}
