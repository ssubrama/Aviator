...
#import "chrome/browser/championconfig/ProtectedUnprotectedViewMac/unprotected_decoration.h" //champion_unprotected (Balaji)
#import "chrome/browser/championconfig/ProtectedUnprotectedViewMac/protected_decoration.h" //champion_protected added
...
LocationBarViewMac::LocationBarViewMac(AutocompleteTextField* field,
    ...
    : LocationBar(profile),
      ...
      unprotected_decoration_(new UnprotectedDecoration(browser)), /* champion_unprotected (Balaji) */
      protected_decoration_(new ProtectedDecoration(browser)), /* champion_protected added (Balaji) */
  ...
  unprotected_decoration_->SetStarred(true); // champion_unprotected (Balaji)
  protected_decoration_->SetStarred(true); // champion_protected added (Balaji)
...
void LocationBarViewMac::SetEditable(bool editable) {
  ...
  UpdateUnprotectedDecorationVisibility(); // champion_unprotected (Balaji)
  UpdateProtectedDecorationVisibility();  //  champion_protected added (Balaji)
...
NSPoint LocationBarViewMac::GetUnprotectedBubblePoint() const {  // champion Balaji
  AutocompleteTextFieldCell* cell = [field_ cell];
  const NSRect frame = [cell frameForDecoration:unprotected_decoration_.get() inFrame:[field_ bounds]];
  const NSPoint point = unprotected_decoration_->GetBubblePointInFrame(frame);
  return [field_ convertPoint:point toView:nil];
}
NSPoint LocationBarViewMac::GetProtectedBubblePoint() const { // champion Balaji
  AutocompleteTextFieldCell* cell = [field_ cell];
  const NSRect frame = [cell frameForDecoration:protected_decoration_.get() inFrame:[field_ bounds]];
  const NSPoint point = unprotected_decoration_->GetBubblePointInFrame(frame);
  return [field_ convertPoint:point toView:nil];
}
...
void LocationBarViewMac::Layout() {
  ...
  [cell addRightDecoration:star_decoration_.get()];
   if(!browser_->profile()->IsOffTheRecord())
      [cell addRightDecoration:unprotected_decoration_.get()]; // champion_unprotected to be showed only unprotected profile (Balaji)
   else
      [cell addRightDecoration:protected_decoration_.get()]; // champion_protected to be showed only protected profile (Balaji)
...
// champion_unprotected (Balaji)
void LocationBarViewMac::UpdateUnprotectedDecorationVisibility() {
 // unprotected_decoration_->SetVisible(true);
}
// champion_protected added
void LocationBarViewMac::UpdateProtectedDecorationVisibility() {
 // protected_decoration_->SetVisible(true);
}
void LocationBarViewMac::mouseMovedUnProc (NSEvent* theEvent)
{
   [field_ mouseMovedUnProc:(NSEvent*) theEvent];		
}
