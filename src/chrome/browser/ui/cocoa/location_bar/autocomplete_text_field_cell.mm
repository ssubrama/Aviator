...
#import "chrome/browser/ui/cocoa/browser_window_controller.h" // champion_unprotected (Balaji)
...
- (BOOL)mouseMoved:(NSEvent*)theEvent // champion_unprotected (Balaji)
           inRect:(NSRect)cellFrame
           ofView:(AutocompleteTextField*)controlView {
  LocationBarDecoration* decoration =
      [self decorationForEvent:theEvent inRect:cellFrame ofView:controlView];
/* if(!isShowUnprotectedBubble)
   if(decoration)
  {
    ...
    decoration->OnMouseMoved(decorationRect);
    return NO;
  }
  ...
  if(decoration)
    {
   	  NSRect decorationRect = [self frameForDecoration:decoration inFrame:cellFrame];
      decoration->OnMouseMoved(decorationRect);
      return NO;
  }
