...
#import "chrome/browser/ui/cocoa/location_bar/location_bar_view_mac.h"
#import "chrome/browser/ui/cocoa/browser_window_controller.h" // champion_unprotected
...
- (void)mouseExited:(NSEvent*)theEvent {
	if(isShowUnprotectedBubble)
   {
	...
	if(controller_)
	{  											
      	[controller_ closeUnprotectedBubble]; 
      	controller_ = nil;      
	 } 
	}
   if(isShowProtectedBubble) // champion_protected added
   {
	BrowserWindowController* controller_ = [BrowserWindowController browserWindowControllerForWindow:browser_->window()->GetNativeWindow()];															
    if(controller_)
	{  			
        [controller_ closeProtectedBubble];
        controller_ = nil;        
	  }	 
	}
if(hoveredButton_)
...
- (void)mouseMoved:(NSEvent*)theEvent {
  ...
  else // champion balaji
  {
    locationBarView_->mouseMovedUnProc(theEvent);
  }
}
...
- (void)setHasToolbar:(BOOL)toolbar hasLocationBar:(BOOL)locBar {
  ...
  if(CHAMPION_INTERNAL_BROWSER | SINGLE_SITE)
     locationBarView_->SetEditable(false); // champion_single_site
   else 
...
- (NSPoint)unprotectedBubblePoint {
	 return locationBarView_->GetUnprotectedBubblePoint(); // champion_unprotected
}
- (NSPoint)protectedBubblePoint {
	 return locationBarView_->GetProtectedBubblePoint(); // champion_protected added
}
