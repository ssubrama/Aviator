...
#endif
prefs::kManagedDefaultReferrerSetting, /* chmapion - Added for manage exception of Referrer header */
 //NULL, //  for referrer champion added  
};
...
void PolicyProvider::RegisterProfilePrefs(
  ...
  registry->RegisterListPref(prefs::kManagedReferrerAllowedForUrls,  // champion - purge referrer
                          user_prefs::PrefRegistrySyncable::UNSYNCABLE_PREF);
  registry->RegisterListPref(prefs::kManagedReferrerBlockedForUrls,
                          user_prefs::PrefRegistrySyncable::UNSYNCABLE_PREF);
  ...
  registry->RegisterIntegerPref(prefs::kManagedDefaultReferrerSetting,  /* Champion - Added for manage exception of referrer header feature */ 
                             CONTENT_SETTING_DEFAULT,
                             user_prefs::PrefRegistrySyncable::UNSYNCABLE_PREF);  
}
...
PolicyProvider::PolicyProvider(PrefService* prefs) : prefs_(prefs) {
  ...
  pref_change_registrar_.Add(                     // champion - referrer
      prefs::kManagedReferrerAllowedForUrls, callback);
  pref_change_registrar_.Add(
      prefs::kManagedReferrerBlockedForUrls, callback);       	
  ...
  pref_change_registrar_.Add(
      prefs::kManagedDefaultReferrerSetting, callback); // champion added for referrer header
}
...
void PolicyProvider::OnPreferenceChanged(const std::string& name) {
  ...
  } else if (name == prefs::kManagedDefaultReferrerSetting) {         /* champion Referrer */
  	UpdateManagedDefaultSetting(CONTENT_SETTINGS_TYPE_REFERRER);  
             ...
             name == prefs::kManagedReferrerAllowedForUrls ||
             name == prefs::kManagedReferrerBlockedForUrls) {
