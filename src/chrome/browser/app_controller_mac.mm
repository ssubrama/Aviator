...
#include "chrome/browser/championconfig/warnbeforequitmac/QuitDialog.h"           // Champion Added for WBQ (Balaji)
#include "chrome/browser/championconfig/warnbeforequitmac/DialogBoxController.h"  // Champion Added for WBQ (Balaji)
#include "chrome/browser/championconfig/RestartUpdateMac/RestartDialog.h"        // Champion Added for RestartAviatorDialog (Balaji)
#include "base/mac/bundle_locations.h" // champion added (Balaji)
#include "chrome/browser/ui/webui/options/browser_options_handler.h" // champion Balaji UserProfileDeleted
...
#include<fstream>
#include<stdio.h> //
bool brestartAviatorToTakeUpdate=false;    // Champion Added for RestartAviatorDialog (Balaji)
bool bshowAviatorRestartDialogAgain=false; // Champion Added for RestartAviatorDialog (Balaji)
bool bQuitDockAndMenu=false;               // Champion Added for WBQ (Balaji)
bool bDownloadInProgressShowed=false;      // champion : WBQ - Download is in progress warning dialog  (Balaji) 25July14
bool bquitFromDock = false; // champion navneet
int Aviator_CloseDialog( const char* header, const char* message, unsigned long message_type )
{
...
Browser* CreateBrowser(Profile* profile) {
  {
    bDownloadInProgressShowed=false;      // champion : WBQ - Download is in progress warning dialog  (Balaji) 25July14
...
- (void)awakeFromNib {
  // champion : Bundle PDF VIEWER Extention for the existinfg Users start (Balaji) 
  NSString *UserID = [self GetUserName];
  NSString *AvaitorPrefLoc = [NSString stringWithFormat:@"%@%@", UserID,@"/Library/Application Support/Aviator/"]; 
  std::string extdir=[AvaitorPrefLoc UTF8String];	    	
  extdir.append("External Extensions/");
  base::FilePath oFilePath(extdir);
  bool bExternalExist=base::DirectoryExists(oFilePath);
  if(!bExternalExist) {
   std::string makedir("mkdir ");
   makedir=makedir+"\""+extdir+"\"";
   //std::ofstream outputfile("/Users/photoninfotech/BALAJI/cpext_app.txt",std::ios::out|std::ios::app);
   //	outputfile<<" The CopyExtension Path is : " <<  makedir << std::endl;
   //	outputfile.close();
   system(makedir.c_str());
   }
  base::FilePath Frameworkpath = base::mac::FrameworkBundlePath(); 
  std::string AvaitorPrefLocPath([AvaitorPrefLoc UTF8String]);
  std::string CopyExtension("cp -fR ") ;
  CopyExtension=CopyExtension+"\""+Frameworkpath.value()+"/External Extensions/"+"\""+" "+"\""+AvaitorPrefLocPath+"/External Extensions/"+"\"";
 // std::ofstream outputfile("/Users/photoninfotech/BALAJI/applicationWillFinishLaunching.txt",std::ios::out|std::ios::app);
	//outputfile<<" The CopyExtension Path is : " <<  CopyExtension << std::endl;
 // outputfile.close();
  system(CopyExtension.c_str());
  // champion : Bundle PDF VIEWER Extention for the existinfg Users end (Balaji) 
...
- (BOOL)tryToTerminateApplication:(NSApplication*)app {
  ...
  if (!browser_shutdown::IsTryingToQuit() && ![self shouldQuitWithInProgressDownloads])
   return NO;
...
- (BOOL)tryToTerminateApplicationAndRestart:(NSApplication*)app {
        bQuitDockAndMenu =true; // champion WBQ balaji  
	    NSString *UserID = [self GetUserName];
	    NSString *UserIDEx = [NSString stringWithFormat:@"%@%@", UserID,@"/Library/Aviator"];   
	    NSString *RunUpdateScript = [NSString stringWithFormat:@"%@%@", UserID,@"/Library/Aviator/RunUpdate_Aviator.sh"];
        std::string strRunUpdateScript([RunUpdateScript UTF8String]);
        base::FilePath FPRunUpdateScript(strRunUpdateScript);
        if (base::PathExists(FPRunUpdateScript))
    	{
    		NSTask *t1 = [[[NSTask alloc] init] autorelease];	  	
	  		try {
				[t1 setLaunchPath:RunUpdateScript];
				[t1 setArguments:[NSArray arrayWithObjects:[self GetAppXPath], UserIDEx, nil]];		
				// Launch (forks) the process
				[t1 launch]; // raises an exception if something went wrong
				[t1 waitUntilExit];
			}
			catch (NSException *exception) {           
            	NSLog(@" RunUpdate_Avaitor exception: %d", [t1 terminationStatus]);
        	}        
         }         
  // Check for in-process downloads, and prompt the user if they really want
  // to quit (and thus cancel downloads). Only check if we're not already
...
//added for champion/Aviator update
...
NSString* AdjustHomedir(NSString* s, const char* home_dir) {
 ...
 -(NSString*) GetUserName {
    ...
    NSString* UserID = [NSString stringWithFormat:@"%s", pwentry->pw_dir];
    return UserID;
}
-(NSString*) GetAppXPath {
    	NSString *separatorString = @"/Aviator.app"; 
    	NSString* bundlePath = [base::mac::FrameworkBundle() resourcePath];   
    	NSScanner *aScanner = [NSScanner scannerWithString:bundlePath];
    	NSInteger anInteger;
    	[aScanner scanInteger:&anInteger];
   	 	NSString *AppXPath;
    	[aScanner scanUpToString:separatorString intoString:&AppXPath];
    	NSLog(@" Application path : %@", AppXPath);
    	return AppXPath;
    }	
/* champion - added for non root user */
-(void)relunchAgentfile:(NSString*)scriptfilePath{
		NSTask *task = [[[NSTask alloc] init] autorelease];
		NSString* bundlePath = [base::mac::FrameworkBundle() resourcePath];
		NSString *scriptPath = [NSString stringWithFormat:@"%@%@", bundlePath,@"/relunchagentfile.sh"];
		NSLog(@" Inside  relunchAgentfile and scriptPath file path= %@",bundlePath);
		std::string strScriptFilePath([scriptPath UTF8String]);
        base::FilePath fileforscriptRelunchAgentPath(strScriptFilePath);	
		if(base::PathExists(fileforscriptRelunchAgentPath))  {
		 try  {
			[task setLaunchPath:scriptPath];
			[task setArguments:[NSArray arrayWithObjects:scriptfilePath, nil]];
			[ task setStandardOutput:[NSPipe pipe]];
			NSLog(@" Setting output to default ouput device");		
		  [ task setStandardInput:[NSPipe pipe]];		
		  NSLog(@" Setting input to default ouput device");		
			[ task launch ]; // raises an exception if something went wrong
			[ task waitUntilExit ];
			NSLog(@"Relunch Agent File - status: %d", [ task terminationStatus ]);
		}
		catch (NSException *exception) {
        NSLog(@"Relunch Agent File - status %d", [ task terminationStatus ]);
      }
   	...
   	NSLog(@"Script file Reluanchagentlife is missing ");
} 
-(void)setAgentInUserLib{
     //Is Admin user?
    uid_t current_user_id = getuid();
    struct passwd *pwentry = getpwuid(current_user_id);
    NSString* bundlePath = [base::mac::FrameworkBundle() resourcePath];
   bool resultDir = false;
   NSLog(@"Inside copyAgentFile");
   NSString *homeDir = [NSString stringWithFormat:@"%s%@", pwentry->pw_dir,@"/Library/LaunchAgents"];
   NSString *EngineDir = [NSString stringWithFormat:@"%s%@", pwentry->pw_dir,@"/Library/Aviator"];
   NSString *EnginePath = [NSString stringWithFormat:@"%@%@", EngineDir,@"/AviatorEngine"];
   NSString *TicketPath = [NSString stringWithFormat:@"%@%@", EngineDir,@"/AviatorTicket"];
   NSString *EnginePathwithDot = [NSString stringWithFormat:@"%@", @"./UpdateAviator.sh"] ;
   NSString *nsAgentFile = [NSString stringWithFormat:@"%s%@", pwentry->pw_dir,@"/Library/LaunchAgents/com.aviator.agent.plist"];
   NSString *nsRelunchAgentFile =nsAgentFile;// [NSString stringWithFormat:@"%s%@", pwentry->pw_dir,@"/Library/LaunchAgents/com.aviatorupdate.agent.plist"];	
   NSString *nsAgentFile1 = [NSString stringWithFormat:@"%s%@", pwentry->pw_dir,@"/Library/LaunchAgents/com.aviator.agent"];
   std::string strhomeDir([homeDir UTF8String]);
   base::FilePath agentDir(strhomeDir);
   std::string stragentFile([nsAgentFile UTF8String]);
   base::FilePath agentFile(stragentFile);
   std::string strEngineDir([EngineDir UTF8String]);
   base::FilePath localEngineDir(strEngineDir);
   std::string strEnginePath([EnginePath UTF8String]);
   base::FilePath localEnginePath(strEnginePath);   
   std::string strTicketPath([TicketPath UTF8String]);
   base::FilePath localTicketPath(strTicketPath);
   NSString *ScriptPathlocal = [NSString stringWithFormat:@"%s%@", pwentry->pw_dir,@"/Library/Aviator/UpdateAviator.sh"];
   std::string strScriptPathlocal([ScriptPathlocal UTF8String]);
   base::FilePath localScriptPath(strScriptPathlocal);
   if (!base::PathExists(agentDir))
    {
      NSLog(@"Creating Dir..");
      resultDir = base::CreateDirectory(agentDir);
      NSLog(@"Dir created ? @%s %d", agentDir.value().c_str(), resultDir);
     }
   if (!base::PathExists(agentFile))
      { 
        NSString *scriptPath = [NSString stringWithFormat:@"%@%@", bundlePath,@"/com.aviator.agent.plist"];
        std::string strScriptPath([scriptPath UTF8String]);
        base::FilePath filename (strScriptPath);
        NSLog(@"scriptPath: %s", strScriptPath.c_str());
        NSLog(@"DestPath: %s", agentDir.value().c_str());
        bool success = base::CopyFile(filename,agentFile);
        NSLog(@"success ? %d", success);
    	CFStringRef PrgArg[2];
    	PrgArg[0] = (CFStringRef)EnginePathwithDot;
    	PrgArg[1] = (CFStringRef)EngineDir;
    	CFArrayRef array = CFArrayCreate(kCFAllocatorDefault,
                           (const void **)PrgArg,
                           2,
                           &kCFTypeArrayCallBacks);
//    CFDictionarySetValue(dict, CFSTR("Program Arguments"), array);
  CFStringRef app = (CFStringRef)nsAgentFile1;
  CFStringRef PrgArguments = CFSTR("ProgramArguments");
  CFPropertyListRef plist = CFPreferencesCopyAppValue(PrgArguments, app);
  if (!plist) {
    CFPreferencesSetAppValue(PrgArguments, array, app);
    CFPreferencesAppSynchronize(app);
    CFRelease(array);
    }
   CFStringRef WorkDir = CFSTR("WorkingDirectory");
   CFStringRef Path = (CFStringRef)EngineDir;
  CFPropertyListRef plist1 = CFPreferencesCopyAppValue(WorkDir, app);
  if (!plist1) {
    CFPreferencesSetAppValue(WorkDir, Path, app);
    CFPreferencesAppSynchronize(app);    
    } 
      }            
   if (!base::PathExists(localEngineDir))
    {
      NSLog(@"Creating Aviator Dir..");
      resultDir = base::CreateDirectory(localEngineDir);
      NSLog(@"Dir created ? @%s %d", localEngineDir.value().c_str(), resultDir);
    }
//     if (!base::PathExists(localEnginePath))
        { 
           NSString *scriptPath = [NSString stringWithFormat:@"%@%@", bundlePath,@"/AviatorEngine"];
           std::string strScriptPath([scriptPath UTF8String]);
           base::FilePath filename (strScriptPath);
           NSLog(@"scriptPath: %s", strScriptPath.c_str());
           NSLog(@"DestPath: %s", agentDir.value().c_str());
           bool success = base::CopyFile(filename,localEnginePath);
           NSLog(@"success ? %d", success);
         }
      if (base::PathExists(localEngineDir)) // Update Ticket 
        { 
           /*
           ./AviatorEngine change -productid com.whitehatsec.aviator -store AviatorTicket -xcpath "/Users/Siva/Desktop"
           --Ticket is copied to every build. default app xpath is /Applications
           1. Get App xpath. 
           2. Update /Resources/Ticket with the execution path and copy Ticket to local library folder.
           */          
    	NSString *separatorString = @"/Aviator.app";    
    	NSScanner *aScanner = [NSScanner scannerWithString:bundlePath];
    	NSInteger anInteger;
		...
		[t1 launch]; // raises an exception if something went wrong
		[t1 waitUntilExit];	
		NSLog(@"Terminationstatus - Updating Ticekt: %d", [t1 terminationStatus]);
		}
		catch (NSException *exception) {
           // NSLog(@"Problem Running Task - Aviator: %@", [exception description]);
            NSLog(@"Terminationstatus -  Updating Ticket Exception raised: %d", [t1 terminationStatus]);
        }
       ...
       if (!base::PathExists(localScriptPath))
        { 
           NSString *scriptPath = [NSString stringWithFormat:@"%@%@", bundlePath,@"/UpdateAviator.sh"] ;
           std::string strScriptPath([scriptPath UTF8String]);
           base::FilePath filename (strScriptPath);
           base::CopyFile(filename,localScriptPath);
         } 
         NSLog(@" Before relaunch Agent File and file path= %@",nsRelunchAgentFile);
         std::string strRelunchAgentPath([nsRelunchAgentFile UTF8String]);
         base::FilePath fileRelunchAgentPath(strRelunchAgentPath);	
         if(base::PathExists(fileRelunchAgentPath))
         {	
         	[ self relunchAgentfile:nsRelunchAgentFile ];
         }
         else
         	NSLog(@"Relaunch Agent file is not present in %s path", strRelunchAgentPath.c_str());
 return;    
}
// champion - adding agent file to local lib folder
...
- (void)applicationDidFinishLaunching:(NSNotification*)notify {
  ...
  //[self setUpdateCheckInterval]; //block by utpal no need to check for google update interval 
  [self setAgentInUserLib]; // champion if we not call this function the auto update will not be trigger - auto update for non admin users
//  [self copyResourceForAviatorUpadte];//added for champion autoupdate
...
- (BOOL)userWillWaitForInProgressDownloads:(int)downloadCount {
  ...
  bDownloadInProgressShowed=true; // champion : WBQ - Download is in progress warning dialog  (Balaji) 25July14
  // 'waitButton' is the default choice.
  int choice = NSRunAlertPanel(titleText, @"%@",
                               waitTitle, exitTitle, nil, explanationText);
  /*if(choice==1) // no button
    bDownloadInProgressShowed = false;
  else
    bDownloadInProgressShowed = true;  */  //champion                           
...
- (BOOL)shouldQuitWithInProgressDownloads {
  ...
  for (size_t i = 0; i < profiles.size(); ++i) {
    ...
    if (download_manager &&
       (download_manager->NonMaliciousInProgressCount() > 0 || DownloadService::NonMaliciousDownloadCountIncognitoProfiles() > 0 )) { // champion navneet
      int downloadCount = download_manager->NonMaliciousInProgressCount() + DownloadService::NonMaliciousDownloadCountIncognitoProfiles();
      if ([self userWillWaitForInProgressDownloads:downloadCount]) {
       ...
       // Browser* browser = chrome::FindBrowserWithProfile(profiles[i], chrome::HOST_DESKTOP_TYPE_NATIVE);
        Browser* browser;
        Browser* browser_otr;
        if (download_manager->NonMaliciousInProgressCount() > 0){
        ...
        if (DownloadService::NonMaliciousDownloadCountIncognitoProfiles() > 0){
        	browser_otr = chrome::FindBrowserWithProfile(
            profiles[i]->GetOffTheRecordProfile(), chrome::HOST_DESKTOP_TYPE_NATIVE); 
        	if (!browser_otr) {
        		NSLog(@" In rathi here 4OTR ");
          	browser_otr = new Browser(Browser::CreateParams(
            	  profiles[i]->GetOffTheRecordProfile(), chrome::HOST_DESKTOP_TYPE_NATIVE));
          	browser_otr->window()->Show();
        	}
        	DCHECK(browser_otr);
        	chrome::ShowDownloads(browser_otr);
        }
        return NO;
      }

      // User wants to exit
      bDIPYesClicked = true; // champion navneet 
      return YES;
    }
  }
...
// #if defined(GOOGLE_CHROME_BUILD) // champion enable report an issue (Balaji)
        case IDC_FEEDBACK:
          enable = NO;
          break;
// #endif
...
- (void)commandDispatch:(id)sender {
  ...
  switch (tag) {
    ...
    case IDC_NEW_WINDOW:
      CreateBrowser(lastProfile->GetOffTheRecordProfile()); // champion
    ...
    case IDC_NEW_INCOGNITO_WINDOW:
      ...
      CreateBrowser(lastProfile);    // champion
      break;
    case IDC_RESTORE_TAB:
...
- (BOOL)applicationShouldHandleReopen:(NSApplication*)theApplication
  ...
  bshowAviatorRestartDialogAgain=false; // Champion Restart Aviator for take update (Balaji)
  bCloseSingleTabAndWindow=false; // Champion Added for WBQ (Balaji) // protected window
  bDIPYesClicked=false; // champion Balaji WBQ -DOWNLOAD IN PROFRESS yes clicked
  bDIPNoClicked=false; // champion Balaji WBQ -DOWNLOAD IN PROFRESS yes clicked
  ...
  if (lastProfile->IsGuestSession() || IsProfileSignedOut(lastProfile))
    ...
    CreateBrowser(lastProfile->GetOffTheRecordProfile()); // champion artf360069 : Clicking on dock with no Windows open In Secure Window (Balaji)
...
- (void)openUrls:(const std::vector<GURL>&)urls {
  ...
  if (!startupComplete_) {
   ...
   // champion artf831888 : Links (including links in mail) need to be opened in Protected mode 
   //and outsource link also should be opened in default browser
  std::vector<GURL>::const_iterator it1= urls.begin();  
  chrome::OpenURLOffTheRecord([self lastProfile]->GetOffTheRecordProfile(), *it1,chrome::GetActiveDesktop());   	 
...
/*  Browser* browser = chrome::GetLastActiveBrowser();
  ...
  launch.OpenURLsInBrowser(browser, false, urls, browser->host_desktop_type()); */ // champion Blocked for artf831888 : Links (including links in mail) need to be opened in Protected mode and outsource link also should be opened in default browser
}
...
- (IBAction)showPreferences:(id)sender {
  if (Browser* browser = ActivateBrowser([self lastProfile]->GetOffTheRecordProfile())) { // champion Open Preferences always Protected while clicking Menu->Preferences... Cmd+,  (Balaji)
    // Show options tab in the active browser window.
    chrome::ShowSettings(browser);
  } else {
    // No browser window, so create one for the options tab.
    chrome::OpenOptionsWindow([self safeLastProfileForNewWindows]->GetOffTheRecordProfile()); // champion Open Preferences always Protected while clicking Menu->Preferences... Cmd+,  (Balaji)
  }
}
...
bool IsOpeningNewWindow() {
 ...
 // Champion Added for RestartAviatorDialog (Balaji) Show dialog of Update mechanism restart browser after getting update Start 
void IsUpdateAvailable() {
 if(!bshowAviatorRestartDialogAgain) {
   NSString* executablePath = [base::mac::OuterBundle() bundlePath];
   std::string AppPath([executablePath UTF8String]);
   AppPath.append("/Contents_update");
   base::FilePath oFileApp(AppPath);
   if (base::DirectoryExists(oFileApp)) {
     RestartDialog*	modalDialog		;
     modalDialog	= nil ;
     NSString*		imagePath		;
	 imagePath=nil;
	if ( !modalDialog ) { 
      modalDialog = [RestartDialog sharedMySingleton] ;
	  if(modalDialog) {
	    [modalDialog showWindow:nil];
