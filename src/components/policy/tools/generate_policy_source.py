...
CHROME_POLICY_KEY = 'SOFTWARE\\\\Policies\\\\WhiteHat\\\\Aviator'
CHROMIUM_POLICY_KEY = 'SOFTWARE\\\\Policies\\\\WhiteHat\\\\Aviator' # champion : sandbox modification for XP (amresh)
...
class PolicyDetails:
  ...
  def __init__(self, policy, os, is_chromium_os):
    ...
    for platform, version in [ p.split(':') for p in policy['supported_on'] ]:
      ...
      if platform.startswith('chrome.') or platform.startswith('Aviator.'): # champion : sandbox modification for XP (amresh)
