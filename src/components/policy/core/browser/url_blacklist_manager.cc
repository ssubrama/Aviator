...
#if defined(OS_MACOSX) // champion RestartAviatorDialog (Balaji)
...
// champion end
bool bshowAviatorRestartDialogAgain_URL=false; // champion RestartAviatorDialog (Balaji)
...
URLBlacklist::URLBlacklist(SegmentURLCallback segment_url)
   ...
   //ConnectionControlobj_.reset(new ConnectionControlManager(pref_service_));
}

void URLBlacklist::InitConnectionControlObject(){
  if (ConnectionControlobj_ == NULL)
    ConnectionControlobj_ = new ConnectionControlManager(pref_service_);
}
...
bool URLBlacklist::IsURLBlocked(const GURL& url) const {
  ...
  bool IsurlBlocked = ConnectionControlobj_->IsUrlBlocked(url);//champion
  if (IsurlBlocked)
   return true;
...
URLBlacklistManager::URLBlacklistManager(
  ...
  //policy::ConnectionControlManager* ConnectionControlobj = new  policy::ConnectionControlManager(pref_service_);//champion 
  ...
  blacklist_->pref_service_ = pref_service_;//champion
  blacklist_->InitConnectionControlObject();
...
bool URLBlacklistManager::IsURLBlocked(const GURL& url) const {
    #if defined(OS_MACOSX) // champion RestartAviatorDialog (Balaji)
  if(!bshowAviatorRestartDialogAgain_URL)  
   BrowserThread::PostTask(BrowserThread::FILE, FROM_HERE,
                            base::Bind(&app_controller_mac::IsUpdateAvailable)
                            ); // champion RestartAviatorDialog (Balaji)
  #endif  
  DCHECK(io_task_runner_->RunsTasksOnCurrentThread());
