...
class RenderFrameHostManagerTest
 ...
 public:
  ...
  TestRenderViewHost* CreateSwappedOutRenderViewHost() {
    const GURL kChromeURL("aviator://foo");
...
// Tests that when you navigate from a aviator:// url to another page, and
...
TEST_F(RenderFrameHostManagerTest,
  ...
  const GURL kChromeURL("aviator://newtab");
  const GURL kUrl1("http://www.google.com");
  const GURL kUrl2("http://www.chromium.org");
...
TEST_F(RenderFrameHostManagerTest, WebUIInNewTab) {
  ...
  const GURL kUrl2("aviator://foo/bar");
...
TEST_F(RenderFrameHostManagerTest, EnableWebUIWithSwappedOutOpener) {
  set_should_create_webui(true);
  const GURL kSettingsUrl("aviator://aviator/settings");
  const GURL kPluginUrl("aviator://plugins");
