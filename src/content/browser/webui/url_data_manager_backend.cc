...
const char kChromeURLContentSecurityPolicyHeaderBase[] =
    "Content-Security-Policy: script-src aviator://resources " // champion 
    "'self' 'unsafe-eval' 'unsafe-inline'; "; //champion : Content Security Policy - Navneet
...
class ChromeProtocolHandler
 ...
 public:
  ...
  virtual net::URLRequestJob* MaybeCreateJob(
    ...
    // Check for aviator://view-http-cache/*, which uses its own job type.
    if (ViewHttpCacheJobFactory::IsSupportedURL(request->url()))
      return ViewHttpCacheJobFactory::CreateJobForRequest(request,
                                                          network_delegate);

    // Next check for aviator://appcache-internals/, which uses its own job type.
    ...
    // Next check for aviator://blob-internals/, which uses its own job type.
    ...
    // Next check for aviator://tcmalloc/, which uses its own job type.
    if (request->url().SchemeIs(kChromeUIScheme) &&
    ...
    // Next check for aviator://histograms/, which uses its own job type.
    if (request->url().SchemeIs(kChromeUIScheme) &&
...
URLDataSourceImpl* URLDataManagerBackend::GetDataSourceFromURL(
    const GURL& url) {
  // The input usually looks like: aviator://source_name/extra_bits?foo
