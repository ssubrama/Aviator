...
/*************************** Aviator implementation for Referrer champion ****************/
bool RenderFrameImpl::RemoveReferrer(GURL HostURL, GURL ReferrerURL) {
  std::string HostStr(HostURL.host());
  std::string ReferrerStr(ReferrerURL.host());
  bool HostIsDomain=IsSameDomain(HostStr,ReferrerStr);// true means check for same domain

  if (render_view_->renderer_preferences_.AlwaysOrNeverSendReferrer) { // Allow Referrer
    if (isPresent(false, ReferrerStr) && !HostIsDomain) // check with Block vector	
      return true;          
  } else { // Block(never send referrer header) referrer
    if(!HostIsDomain && !isPresent(true, ReferrerStr))
      return true;
  }
...
bool RenderFrameImpl ::isPresent(bool Allow_Or_Block,std::string referrerurl) {	
  std::vector<std::string>::iterator position;	

  if(Allow_Or_Block) {
    position = std::find(render_view_->renderer_preferences_.allow.begin(), render_view_->renderer_preferences_.allow.end(), referrerurl);
    if(position !=render_view_->renderer_preferences_.allow.end())
      return true;
    else
      return false;			
  } else {
    position = std::find(render_view_->renderer_preferences_.block.begin(), render_view_->renderer_preferences_.block.end(), referrerurl);
    if( position !=render_view_->renderer_preferences_.block.end())
      return true;
    else
      return false;			
  }		
...
std::string RenderFrameImpl::GetDomain(std::string host_url) {
  size_t start=0,second=0;
  unsigned count =0;
  for(unsigned int i=0; i < host_url.length(); i++) {
    if(host_url[i] == '.') {
      if( 0 == count ) {
        start=i;
        count=count+1;
  ...
  start=start+1;
  second=(second - start); //total length of sub string not second index
  std::string host_temp=host_url.substr(start,second);
  return host_temp;
}

bool RenderFrameImpl::IsSameDomain(std::string host_url,std::string referrer_url) {
  if(host_url.length() > 0 && referrer_url.length() > 0) { //both length should not zero
    std::string temp_host_url=GetDomain(host_url);
    std::string temp_referrer_url=GetDomain(referrer_url);
    if(temp_host_url.compare(temp_referrer_url) !=0 )
      return false;
    else
      return true;
  } else
    return false; //null string otherwise it will crash in GetDomain function
}	
...
void RenderFrameImpl::willSendRequest(
  ...
  /* champion for referrer */
  WebString referrerWStr = request.httpHeaderField("Referer");
  GURL HostURL = GURL(request.url());
  GURL ReferrerURL = GURL(referrerWStr);
  std::string ref(ReferrerURL.host());
  if (RemoveReferrer(HostURL, ReferrerURL))
